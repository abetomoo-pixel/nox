-- ═══════════════════════════════════════════════════════════════════════════
-- mig0098: R2-a 延長メニュー複数＋開卓時ルール手動選択（自己検証版）
--   checks.ext_menu_snap ＋ check_open 拡張（snap 凍結・p_set_rule_id・set 行 block_no=0＝R2-7c）
--   ＋ check_extension_add 拡張（p_rule_id・snap 解決）
--   底本 = nox_mig0098_live_defs.sql（sha256 cfa536d7…6e21・live 逐語）
--   正本 = R2 設計書 v1.1（R2-1〜R2-5・R2-7c・裁定33/34）
-- ─────────────────────────────────────────────────────────────────────────────
-- ★非冪等（本番手貼り1回・再実行厳禁）: add column／旧署名 drop 2本
-- ★notify pgrst はファイル外・手貼り後に単発
-- ★pricing_resolve_core は不触（pin 保全）＝鏡像 where の相互参照は check_open 側コメント＋設計書が正
-- ★教訓22 消化: time_auto=true 行の INSERT 経路は check_open（set・本 mig で block_no=0 化）と
--   check_time_charge_apply（0097/0097b 対応済み）の2本で全数＝null 再生産経路は本 mig で根絶
-- ★ゲート（0057 5腕・billing・not open・has payments・auto mode・seat 行ロック・open 部分ユニーク
--   conflict）は raise/句比較で逐語不変を機械確認済み
-- ★アリティ変更2本＝旧署名 drop＋ACL 再適用（0095 前例）。app 呼び出しは named 引数＝無改修互換
-- ═══════════════════════════════════════════════════════════════════════════

begin;

-- ─────────────────────────────────────────────
-- (1) DDL: checks.ext_menu_snap（NULL可＝旧伝票は null・app は null で既定のみ表示）
-- ─────────────────────────────────────────────
alter table public.checks add column ext_menu_snap jsonb;

-- ─────────────────────────────────────────────
-- (2) check_open: アリティ 4→5（旧署名 drop＋ACL 再適用）
-- ─────────────────────────────────────────────
drop function if exists public.check_open(uuid, integer, text, uuid);
...
revoke execute on function public.check_open(uuid, integer, text, uuid, uuid) from public, anon;
grant  execute on function public.check_open(uuid, integer, text, uuid, uuid) to authenticated, service_role;

-- ─────────────────────────────────────────────
-- (3) check_extension_add: アリティ 1→2（旧署名 drop＋ACL 再適用）
-- ─────────────────────────────────────────────
drop function if exists public.check_extension_add(uuid);
...
revoke execute on function public.check_extension_add(uuid, uuid) from public, anon;
grant  execute on function public.check_extension_add(uuid, uuid) to authenticated, service_role;
