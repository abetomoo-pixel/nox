-- =============================================================
-- mig0129: 裁定119 — 適用セットルールの凍結(set_rule_id/set_rule_name)
--   checks へ純スナップ2列(FK なし=pricing_rules は物理削除 RPC が存在するため。
--   スナップ思想: 削除・改名は非遡及)。check_open は override 確定後の r_set.rule_id から
--   名称を凍結。フォールバック(rule 0件)=両列 null。既存伝票=null(0129 以前と区別なし)。
--   UI 表示規則(裁定119): set_rule_name not null のときのみ「適用: {name}」バッジ・
--   name null ルール/フォールバック/旧伝票=非表示。
--   前提: mig0128 適用済み。冪等: 可。同 arity=DROP なし(OR REPLACE のみ)。
--   golden 6値不変 gate。A6 名簿: 変更なし(再作成のみ)。
-- =============================================================
begin;

-- 0) fail-fast: 0128 適用済み確認
do $mig$
begin
  if not exists (
    select 1 from pg_proc p join pg_namespace n on n.oid = p.pronamespace
     where n.nspname = 'public' and p.proname = 'check_open'
       and pg_get_function_identity_arguments(p.oid) like '%p_category_id uuid') then
    raise exception 'mig0129 precondition: check_open 6引数版なし(0128 未適用)';
  end if;
end $mig$;

-- 1) checks へ凍結列(FK なし・純スナップ)
alter table public.checks add column if not exists set_rule_id uuid;
alter table public.checks add column if not exists set_rule_name text;

-- 2) check_open: 凍結2行追加(0129 起草直前 live 逐語ベース・変更点は ★mig0129 のみ)
...
-- ★live 実測再現: anon grant なし(OR REPLACE で ACL は保持されるが明示再設定=防御深度)
