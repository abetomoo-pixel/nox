-- 0141_check_open_gate_fix.sql  (C層③ 補正・裁定 C③-15 追記・教訓68・2026-09-10)
-- 前提: 0140 適用済み。0140 は書き換えない。
-- 内容: check_open の関所行の変数誤り(v_store は未宣言=開卓が全件 column "v_store" does not exist)を補正。
--       誤行(ゲート行直後の L28)を削除し、v_seat 確定後(`if v_seat.id is null … forbidden` の直後)に
--       `perform public.assert_day_open(v_seat.store_id, public.biz_date_of(v_seat.store_id, now()));` を 1 行。本文は他を一字も変えない。
-- 生成: docs/tmp/gen_0141_check_open_gate_fix.cjs が live pg_get_functiondef から機械生成(挿入行を除くと 0140 適用前の本文と一字一致を assert)。
-- 生成時 proof: select 'nox-project-proof', count(*) from public.orgs → 3
-- 手貼り: SQL Editor で Ctrl+A → Run。末尾 proof で has_v_store=false / assert_day_open_callers=16 / open_perform=1 を確認。
begin;

-- ---------- check_open（削除: 旧 L28 の v_store 行 ／ 挿入: L43・v_seat 確定の直後。ACL は create or replace で保持）----------
...
union all select 'has_v_store', (prosrc like '%v_store%')::text from pg_proc where pronamespace='public'::regnamespace and proname='check_open'
union all select 'open_perform', (select count(*) from regexp_matches(prosrc, 'perform public\.assert_day_open\(v_seat\.store_id, public\.biz_date_of\(v_seat\.store_id, now\(\)\)\);', 'g'))::text
  from pg_proc where pronamespace='public'::regnamespace and proname='check_open'
...
-- 期待: proof=3 / has_v_store=false / open_perform=1 / assert_day_open_callers=16 / gate_shape_f=0140 適用後と同じ(123)
