-- mig0068: キャストドリンク注文時帰属 (c) check_close 按分ループの除外条件
-- 前提: mig0066 / mig0067 適用済み / 冪等（create or replace）
-- ★実差分は for ループの select に not exists 条件2行を足しただけ。
--   他は live pg_get_functiondef の全文を一字も変えずに再掲している。
    ...
    for v_line in
      select * from public.check_lines
       where check_id = p_check_id and product_id is not null
         ...
         -- ★mig0068: キャストドリンクは按分から除外（バックは drink_claims 経路で帰属）
         and not exists (select 1 from public.products p
                          where p.id = check_lines.product_id and p.back_exempt_from_split)
    loop
