...
- 玲奈ゴールデン2系統（verify:nox-pay T1a=pt除外5170 / T1b=モック忠実5931）。正は本指名商品pt 込み。
  源泉ゴールデンは T1a=117241 / T1b=125802（裁定26 で 121836/130397 から張り替え＝計算期間日数×切捨・periodDays=31）。
- F2 差し替え点（源泉日数・二重控除ガード・売上率テーブル・丸め）は pay.ts 冒頭コメントに記録済み。

## verify

- 機能完了ごとに verify スイート緑を確認してからコミット（`npm run verify:f0` = 17本：pay / shift-time / punch-match / receipt / anon-guard / rls / grants / payroll / payroll-csv / inventory / product-bulk / pricing / pricing-apply / categories / cast-photo / labor-forecast / rate-back）。
...
- **提案に番号を振る時点で台帳に本文を残す**（2026-09-10: 裁定238 は本文なしで引き継ぎ v29 に「提案中」とだけ載り、CC が台帳・docs・Downloads を探して見つからなかった）。
  番号＝台帳の見出し。本文が無い番号は作らない。承認前は見出しに「（提案中）」を付け、承認時に日付へ差し替える。
