 ...
 *  逆テスト 2 本（手動・各 1 回）: 食品除外を外す（CLASS_TO_TYPE.food を 'drink' に）→su(2-3) 赤／写像順を変える（hours を settings より前に）→su(2-1) 赤。
 */
...
const planOf = (biz: BizType, includeProducts = true) => buildSetupPlan({
  storeId: "00000000-0000-4000-8000-000000000001", biz, storeName: null, hours: hoursOf(templateOf(biz)), systems: systemsAll, includeProducts,
...
const EXPECT: Record<BizType, { products: number; included: number; food: number; seats: number; rules: number; comp: number; overrides: number }> = {
  cabaret: { products: 67, included: 60, food: 7, seats: 8, rules: 2, comp: 1, overrides: 54 },
  girlsbar: { products: 34, included: 30, food: 4, seats: 14, rules: 0, comp: 1, overrides: 30 },
  snack: { products: 28, included: 20, food: 8, seats: 11, rules: 0, comp: 1, overrides: 0 },
  lounge: { products: 40, included: 37, food: 3, seats: 9, rules: 1, comp: 1, overrides: 37 },
...
for (const biz of ["cabaret", "girlsbar", "snack", "lounge", "bar"] as BizType[]) {
  ...
  check(`su(2-3) ★${biz}: 商品 ${e.products} 件中 投入 ${e.included}・食品等除外 ${e.food}（271-9）`, (t?.products.length ?? 0) === e.products && pp.included.length === e.included && pp.excludedTotal === e.food && excludedFoodCountOf(biz) === e.food, JSON.stringify({ all: t?.products.length, inc: pp.included.length, ex: pp.excluded }));
  ...
  check(`su(2-5) ${biz}: bulk の item は name/type/price/cost/category の 5 キー・type は drink/champ/bottle`, !bulk || ((bulk.args!.p_items as Array<Record<string, unknown>>).every((it) => JSON.stringify(Object.keys(it)) === JSON.stringify(["name", "type", "price", "cost", "category"]) && ["drink", "champ", "bottle"].includes(String(it.type)))));
