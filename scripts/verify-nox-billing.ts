...
async function main() {
  ...
  // ══════════════════════════════════════════════════════════
  {
    ...
    // ★mig0125（裁定112・2026-09-02）: cast_unavailable_set/remove・shift_bulk_set_daily（ゲート内蔵）を
    //   A5 へ・cast_unavailable_list（STABLE 読取・非ゲート）を B(f) へ収載＝対象 108→111・全数 206→210。
    //   shift_set は 6→7引数化（名前不変＝本数不動・旧署名 DROP 済み）。
    //   この +4 も f0 実走の教訓21 assert（live 全数=A∪B）が名簿漏れとして検知→収載した実例（4例目）。
    // ★mig0126（裁定114・2026-09-02）: shift_confirm_bulk（ゲート内蔵）を A5 へ収載＝対象 111→112・
    //   全数 210→211。この +1 も教訓21 assert が検知→収載した実例（5例目）。
    // ★mig0127（裁定116-1・2026-09-02）: set_pricing_category（ゲート内蔵）を A6 へ収載＝対象 112→113・
    //   全数 211→212。教訓21 の先回り収載（f0 実走検知を待たず名簿と pin を同時更新した初例）。
    // ★mig0135（C層①・2026-09-09）: flag_set（ゲート内蔵・owner 限定）を A8 へ・flag_enabled（STABLE 読取・非ゲート）を B(f) へ収載＝
    //   対象 113→114・除外 101→102・全数 214→216。監査書込 2 本は p_reason 末尾追加＝名前不変で本数不動（旧 signature DROP 済み）。
    // ★mig0137（C層② 補正・2026-09-09・裁定233）: 書込 6 本（staff_pattern_set/delete・staff_shift_propose/override/confirm・
    //   staff_deadline_set）に課金ゲート逐語行（auth_org_id() 版）を挿入し A8 へ収載＝対象 114→120・除外 115→109・全数 229 不変。
    //   staff_wish_set は B(i)（黒服本人の事実記録＝cast の shift_wish_submit 同型）。
    // ★mig0138（C層③・2026-09-10・裁定 C③-9/C③-11）: check_merge を A1・report_reopen／cash_diff_approve を A8 へ収載（ゲート内蔵 3 本）＝
    //   対象 120→123・除外 109→114（auth_staff_can_close／can_reopen・report_can_close／can_reopen・assert_day_open）・全数 229→237。
    //   payroll_reopen（5 引数）・set_staff_perms（7 引数）・daily_report_reclose（8 引数）は名前不変＝本数不動。0140（関所 16 本）はゲート行不変＝不動。
    ...
    // ★mig0154（裁定294／295・2026-09-24）: punch_correction_request／decide／ack（A5）・set_cast_employment（A10）＝ゲート内蔵 4 本を A へ・
    ...
    check("段47-1 正本の対象134名を読めた", docTargets.size === 134, `got ${docTargets.size}`);
    ...
    check("段47-1 正本の除外120名を読めた", docExcluded.size === 120, `got ${docExcluded.size}`);
    ...
    check("段47-1 live のゲート済み関数 = 134本", liveGated.size === 134, `got ${liveGated.size}`);

    const missing = [...docTargets].filter((n) => !liveGated.has(n));
    const extra = [...liveGated].filter((n) => !docTargets.has(n));
    check("段47-1 ★対象→live: 正本の123本すべてにゲートが入っている", missing.length === 0, missing.join(","));
    ...
    check("段47-1 述語を参照する関数 = 135（134 ＋ ラッパ自身）", refs[0].n === 135, `got ${refs[0].n}`);
    ...
    check("段47-1 挿入行の形が全134本で規約どおり（引数は v_org / auth_org_id() の2種のみ）", shapes[0].n === 134, `got ${shapes[0].n}`);
  }
      ...
      const { rows: armAll } = await db.query(`
        select count(*)::int as n from pg_proc p join pg_namespace n on n.oid=p.pronamespace
         where n.nspname='public'
           and p.prosrc like '%coalesce(public.auth_org_id(), public.auth_kiosk_org_id())%'
           and p.prosrc like '%billing_writable_of(v_org)%'`);
      check("段47-3 ★kiosk 腕を持つ対象17本すべてが v_org 引数でゲート済み（0089 extension・0090 set_people・0091 line_set_group・★0148 check_add_referral 込み）", armAll[0].n === 17, `got ${armAll[0].n}`);
    }
