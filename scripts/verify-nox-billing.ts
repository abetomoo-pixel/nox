...
async function main() {
  ...
  // ══════════════════════════════════════════════════════════
  {
    ...
    check("段47-1 正本の除外121名を読めた", docExcluded.size === 121, `got ${docExcluded.size}`);
      ...
      const { rows: armAll } = await db.query(`
        select count(*)::int as n from pg_proc p join pg_namespace n on n.oid=p.pronamespace
         where n.nspname='public'
           and p.prosrc like '%coalesce(public.auth_org_id(), public.auth_kiosk_org_id())%'
           and p.prosrc like '%billing_writable_of(v_org)%'`);
      check("段47-3 ★kiosk 腕を持つ対象18本すべてが v_org 引数でゲート済み（0089 extension・0090 set_people・0091 line_set_group・★0152 check_referral_set／remove 込み・0148 check_add_referral は drop）", armAll[0].n === 18, `got ${armAll[0].n}`);
    }
