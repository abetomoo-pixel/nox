...
async function main() {
  ...
  check("ss(4-0) live の set_store_profile 白名単を読めた（22 キー＝0147 の 20＋0151 の slide_apply＋0154 の settlement_presets）", live.length === 22 && live.includes("slide_apply") && live.includes("settlement_presets"), `${live.length}: ${live.join(",")}`);
