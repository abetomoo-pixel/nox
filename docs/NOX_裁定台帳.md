...
**追補1（本便で確定・相談役ブロック 2026-09-24・社労士 S-1／S2 の反映・逐語）**: 「裁定291 追補1（2026-09-24・社労士 S-1／S2 の反映）
1. 出退勤の時刻は雇用・委託を問わず常に実時刻で記録する（punches）。「出勤」区分を押したら in を書く（291-3）・退勤も同じ重み。記録の省略は設定できない。
2. 291-1 の店設定は「勤務時間の計算方式」ではなく「給与の時間ルール」work_pay_rule とし 3 値: 'actual'（実働払い＝現行・欠損時の既定）／'shift_guarantee'（確定シフトの時間を下限として保証＝実働がシフトより短くても差し引かない・超過は実働）／'shift_fixed'（確定シフトの時間どおり・遅刻／早上がりは罰金で調整）。
3. 'shift_fixed' は委託キャストにのみ適用。雇用区分のキャストは店設定に関わらず 'actual' で計算し、給与画面に「雇用キャストは実働で計算」の注記を出す。'shift_guarantee' は雇用にも可。
4. 罰金（penalty_config）と不就労控除は台帳上の区分を分ける（雇用キャストの罰金は労基法 91 条の上限管理が要る＝第2期・ローンチでは雇用キャストに罰金を設定したとき warn）。
5. 「雇用／委託」は契約区分であり法的判定ではない旨を、待遇画面の区分選択と利用規約（L6）に明記。
6. 291-1・291-2 は本追補で置き換える。0154 の白名単キーは work_pay_rule・新規店のウィザード既定は 'shift_guarantee'（多数派の運用に近く雇用でも可）。」
＝291-1（work_hours_basis 2 値）・291-2 は本追補で置換（キー work_pay_rule・3 値・欠損 'actual'・ウィザード既定 'shift_guarantee'）。
