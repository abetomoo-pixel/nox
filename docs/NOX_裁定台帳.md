...
| 42 | **待遇画面の準備中項目（C5）** | 裁定101 §2 の器なし11項目（日給制/保証時間帯/判定単位 半月・日/pt付与ルール/粗利基準/延長・昇格バック/帯歩合%/丸め2軸/未達根拠確認記録/達成 params 拡張/率方式=R-2b 後）。**器は作らず準備中バッジで明示**。解錠は項目ごとに別裁定（正本＝`NOX_U2待遇収斂設計書v1.md` §3） |
### 未裁定・消し込み待ち
