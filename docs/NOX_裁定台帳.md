...
R16 席の並べ替えが非原子（set_seat 2 回呼びで sort_order を交換）。本番運用前に reorder RPC（1 トランザクションで 2 行更新）へ移す。mig 1 本。現状は dev の席数が少なく実害が小さいため B レーンでは client のまま通した（★2026-09-14 追記＝D レーンへ）
