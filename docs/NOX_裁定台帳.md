...
- **#66 起票（低・2026-09-09・修正しない）**: 各画面の console「resource 400」の正体＝月末を `${ym}-31` 固定で `lte` する既存クエリが 30 日月／2 月に Postgres 22008（`date/time field value out of range: "2026-09-31"`）で落ちる。捕捉 URL（fetch フックで各 1 回・manager ログイン）: **/report**＝`rest/v1/ar_collections?select=amount&store_id=eq.0f748408…&biz_date=gte.2026-09-01&biz_date=lte.2026-09-31`（`report-board.tsx:233`・E8-2 #12 今月回収 KPI＝該当月は ¥0 表示に落ちる）／**/casts**＝`rest/v1/attendance?select=cast_id,status&date=gte.2026-09-01&date=lte.2026-09-31`（`casts-board.tsx:137`・今月出勤者 KPI＝該当月は 0 名に落ちる）／**/receivables** はルート無し＝売掛タブの 400 は /report と同一 URL。壊れた img は 0。B2 の新規 8 本は無関係（すべて 200）。月末は翌月 1 日 `lt` へ寄せるのが筋（B2-b の D37/D47 は同方式で実装済み）＝修正は別裁定。
---
