...
正本＝`docs/NOX_E8裁定_v1.md`（マトリクス＝`docs/tmp/e8_gap_matrix.md` に全134件を転記済み）。
...
調査は `docs/tmp/dp3_structure_survey.md`（DP3-S）。
...
**LINE レーン（T3）**へ。申し送りの本文は `docs/tmp/dp3_structure_survey.md` の末尾。
...
### DP-R 全画面再監査（`docs/tmp/dpr_audit.md`）
...
全数走査の結果（`docs/tmp/r2_select_scan.md`）: `<select>` は全 **81本**。
...
### 教訓31〜34：欠番（本セッションでは新規に確定していない）

台帳・`docs/` 配下とも **教訓31〜34 の記述は0件**（実測）。前セッションからの積み残しも存在しなかったため、
**番号を詰めずに欠番として残す**（詰めると後から書かれた参照が別の教訓を指してしまう）。

★**宙吊りの参照が1件ある**：`app/(manage)/shift/shift-board.tsx:162` のコメントが
「`★SC-8 ③-0（教訓33）`」と**台帳に実体の無い教訓33 を参照**している
（内容は「面をまたぐ1本の state を `tab` 判定で排他すると、条件を1つ落としただけで2面が同時に出る」）。
**教訓として登録するか、コメントから番号を外すか**を次のクローズで裁定する（コードは本収載では触っていない）。
