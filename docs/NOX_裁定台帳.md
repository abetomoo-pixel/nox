...
## 裁定6：キオスク作らない（Agoora・2026-07-17・射程は★要確認のまま）

追記3 の列挙より逐語収載。**射程の更新（2026-07-17 実装再開指示）**：
「レジ用キオスク＝作らない。打刻キオスク F4a（mig0043・app/kiosk・kiosk_punch）は別物・実装済み・維持」
—— ★ただしこの射程解釈は**相談役の理解であり Agoora 未確認**。「要確認」フラグ付きで運用する
（＝レジ用キオスクの新規実装はしない・F4a には触らない・射程確定まで撤去もしない）。
...
## 裁定8：モック↔repo 欠落の全件裁定＝フェーズN1 確定（Agoora 全件確定・2026-07-17）
...
実装順序（固定）：N1-a（DB 非改変）＝E5→A1→A2 ／ N1-b（mig レビュー先行）＝E1→B4→B1→B2→C1→A4→B6 ／
...
- レジ用キオスク＝作らない（射程は裁定6 参照・★打刻キオスク F4a は別物・実装済み・維持＝Agoora 要確認フラグ付き）。
