...
## 裁定293（相談役ブロック・2026-09-24）精算調整（本文＝受領待ち・追補1 のみ先に収載）

出典＝相談役ブロック 2026-09-24（便 TX の途中で受領・裁定293 の本文は未着＝届き次第この節へ逐語で差し込む。番号 293 は本追補で確定・次の裁定番号は 294）。
...
＝旧 291-1〜291-6 は本追補で置換（work_hours_basis は作らない・0154＝punch_corrections 表＋punch_correction_request／decide＋cast_plan 白名単 3 キー＋91 条検査＋penalty の適用区分）。1 版目（work_pay_rule 3 値・punch_set／punch_delete）は無効。
