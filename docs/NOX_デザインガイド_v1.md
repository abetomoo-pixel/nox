...
| `--champ` | `#f0cf82` | **（無）** | **＝gold2 別名**（E5 裁定・E5a 実測コントラスト比 1.04 による視覚統合。名前・参照は不触） | 65 |
...
| `--bad-bg` | `#2C1B1B` | ★非モック由来（E5c・I1 裁定）。警告面の地＝theme.ts alert の実使用10箇所が出典 | alert 経由 10 |
| `--bad-bd` | `#5A2E2E` | 同上・警告面の枠線。casts/staff の danger ボタン枠 2箇所も参照化 | alert 経由 10＋2 |
| `--bad-ink` | `#F0B9B9` | 同上・警告面の上の文字（`--bad` は面の上では暗すぎる） | alert 経由 10 |
...
| # | 対象 | 裁定 | 実装 |
|---|---|---|---|
| 1 | `--champ`（65参照） | **E5 で再裁定済＝champ は gold2 の別名**（2026-08-17・暫定解消） | 値を `#f0cf82`（`--gold2` 同値）へ。旧 `#E6D6A8` との実測コントラスト比 **1.04**（E5a）＝視覚上ほぼ同色のため、モックが持たない3段目を廃し**2段へ視覚統合**。トークン名と65箇所の参照は不触（別名として存置）。印刷側 `@media print` の `--champ:#000` 上書きも従来どおり |
...
| 金（主） | `--gold` | `--gold` | 同名 |
| 金（明） | `--gold2` | `--gold2` | 同名 |
| 金（淡） | **（無）** | `--champ` | E5 裁定で `--gold2` 別名（値同値・名前のみ独自） |
| 文字（主） | `--text` | `--ink` | |
| 文字（副） | `--muted` | `--sub` | |
| 状態（良） | `--green` | `--ok` | |
| 状態（悪） | `--red` | `--bad` | |
...
`docs/dp/e4_gaps.md` に記録した3件の裁定を反映した。
...
#### 11-5-DP2. ★是正辞書の追補（DP2 T2・2026-08-21・裁定 DP0-3）

**上の「残ゼロ」は成立していなかった**（DP0 調査で実測）。原因は **走査辞書が4色に閉じていたこと**と、
**走査範囲が `.ts/.tsx` に閉じていたこと**の2点。DP2 T2 で機械走査して全数を是正した。

**(1) 辞書漏れ＝「金の暗面」3色（非モック由来・トークン不在だった）**

| 値 | 是正後 | 件数 | 内訳 |
|---|---|---|---|
| `#1F1B12` | `var(--goldface)` | **6** | casts-board:404／customers-board:416／pricing-board:560／time-pricing-panel:28／payroll-board:391／`theme.ts` の `logo` |
| `#1B1710` | `var(--goldface2)` | **8** | customers-board:621・:759／mine/ranking:54／cast-picker:75／**globals.css の `.nox-chip.on` / `.nox-subnav button.on` / `.nox-cald.sel` / `.nox-navsheet-i.on`** |
| `#14120C` | `var(--goldface3)` | **2** | pricing-board:560／time-pricing-panel:28 |

→ 計 **16箇所**。トークン3本は `globals.css .nox-dark` へ新設（`--goldbg` に揃えた命名・明→暗の順）。
**値は従来と同一＝見た目は不変**。E5c I1 の「実使用3箇所以上でトークン新設」基準を満たす。

**(2) 4色辞書の取り残し 2件（「残ゼロ」の反証）**

- `theme.ts logo` の `linear-gradient(135deg,#1F1B12,#0B0B0F)` に **旧 `--bg` `#0B0B0F` が live で残存**。
  E4 群4 は同じ値を3箇所是正しながらこの1件を落としていた。→ `var(--bg)` へ。
  **暗端が `#0B0B0F`→`#080808` へ 2〜3 階調暗くなる**＝意図した是正。
- `globals.css .nox-medal.g1` の `linear-gradient(135deg,#F0D58A,#C9A24A)` に **旧 gold `#C9A24A` が live で残存**。
  E4 群4 の走査は `.tsx` 中心で **`globals.css` を対象にしていなかった**。→ `var(--gold)` へ。

**(3) 是正後の残置＝意図的3件 ＋ 未裁定 13行（要裁定）**

- **`.ts/.tsx` の live 生 hex は 3件のみ**（コメント除去して機械走査）＝
  `viewport.themeColor #080808`（メタデータは CSS var 不可）／`app/r/[token]` の `#f4f2ee` `#1a1a1a`
  （**白地黒字の帳票トーン＝画面パレット対象外**の R2-c 裁定）。
- **`globals.css` の live 生 hex は 18行**。うち **5行は `@media print` の `#fff`/`#000`**＝印刷パレット（意図的）。
  残る **13行はいずれも 1〜2回しか使われない一点物**（`#1A1820` 2回・他は全て1回）で、
  **E5c I1 の「実使用3箇所以上」基準を満たさない**ため DP2 では**トークン化しない**（発明しない原則）。
  内訳＝`.nox-seat.busy` の `#4A4436`/`#1A1820`／サイドバー地 `#0d0d0c`・hover `#171715`・群見出し `#66635d`／
  `#211D14`+`#504323`／`#5B4927`+`#211B10`+`#EAD28E`／メダル銀銅 `#E6E6EC`+`#A6A6B2`+`#E0B27A`+`#B07B45`
  ＋金メダル明端 `#F0D58A`／`#ECECEF`／在庫バー `#8A5A2B`。
  ※このうちサイドバー地・hover 地・入力地は **ガイド §8-2/§9-1 で「トークンに無い中間色」と
  既に明記済み**＝意図的な一点物。**要裁定**（トークン新設 / 最近傍トークンへ丸め / 現状維持）。

### 11-6. E5/E6 の裁定（レジ・給与・日報・帳票・kiosk）

| # | 対象 | 裁定 | 実装 |
|---|---|---|---|
| 裁定0 | レジのビュー構成差・浮遊 toast | ~~**不追随**（presentation-only を超える構造変更＝別レーン）~~ → **★DP1-⑤ で上書き（下記 11-6-DP1）** | 現行のタブ構成・toast 位置を維持（E5a）→ レジのビュー構成差は DP1 P2 で追随済み |
...
#### 11-6-DP1. ★E5 裁定0 の上書きと DP1 の追加裁定（2026-08-21）

**#7.5 デザイン移植レーン（DP）** の裁定により、本節の一部は**効力を失った／条件が変わった**。
本書は E レーン（E1〜E6）の正本であり続けるが、**レジの構成差については DP1 が上位**になる。

| 旧裁定 | 上書き後（DP レーン） | 根拠 |
|---|---|---|
| **裁定0「レジのビュー構成差は不追随」** | **★DP1-⑤ で上書き＝追随する**。`mock/pages-2026-08` は **構造 canonical**（DP0-4 改訂）で、DP1 は実装の IA・レイアウト・動線をモックへ寄せるレーンだから。ただし **v3 動線（卓選択→全画面切替・「← フロア」・会計後自動復帰）が優先**（DP1-①） | E5 当時は「presentation-only を超える＝別レーン」という理由での**保留**であって恒久の不追随ではなかった。その「別レーン」が DP である |
| （裁定0 の続き）浮遊 toast | **据え置き**（DP1 でも見送り継続） | 位置の 27箇所は依然として構造差。DP1 の対象に入れていない |
| **I2 avatar の HSL 生成色** | **据え置き＝恒久対象外**（★DP0-2 で再確認） | identicon の決定性が意匠。DP2 の是正対象は**分析ビューの内部用語のみ** |

**DP1 P2 でレジに実施した内容（b 6件）**＝タイル列数規約（`.nox-seatgrid.floor` 8/4/2）／
伝票取消のモーダル化／sectionhead 2段組／商品をクリア／**会計3段化（表示段のみ）**／支払の併用導線。
**c 1件（伝票表示中もフロアを残す）は不採用**＝v3 の「選択＝伝票全面・フロアは描画しない」を維持。
★`check_pay` / `check_close` は関数本体の sha256 照合で**逐語不変を機械証明**済み
（pay `bf10898636c3` 824B・closeCheck `25b2affa3e49` 1080B・kiosk 側も同様）。

**★DP1-⑪（2026-08-21・承認済み）: 支払方法の3択は非追随・NOX の 4値を維持する**

モック register-pos は「現金／カード／**併用**」の3択だが、NOX の
`cash / card / ar / other` は減らせない。理由は**3経路に直結している**ため:

1. `payments_method_check`（CHECK 値域）
2. `check_pay` のハードコード検証（`not in (...)`）
3. `daily_report_aggregate` の名指し集計 ＋ `daily_reports` の凍結列 ＋ `report-board.tsx` の再集計

★最大の罠は 3 で、**新しい語彙は `other` にも落ちず日次サマリからサイレント欠落する**
（一方 `report-board` は else other に落ちるため、プレビューと確定値がズレる）。
またモックの「併用」は**支払方法の3つ目ではなく「分けて払う」操作**であり、
NOX は `payments` を複数行にすることで**機構としては既に実現している**。
→ **4値は維持し、併用の導線を言葉で可視化する**（入金モーダルの注記＋既存の均等割り）。

**★register#1（領収書ダイアログ）は後送り台帳から落とす**: `NOX_E8裁定_v1.md` §2「後送り」は
`register#1 領収書ダイアログ（分割・宛名・QR＝L 級・感熱印字で当面回る）` を挙げているが、
**R2-c（mig0099・2026-08-20 `71c134c`）で全実装済み**（発行モーダル＋発行台帳 `/receipts`＋
匿名公開ページ `/r/[token]`＋QR）。裁定書は R2-c 以前の文書のため、**本追記をもって解消済みとする**。

**★純増起票候補（DP1 P2 の d 3件・実装しない）**: モックにあり NOX に**データが無い**もの。

| # | 項目 | 欠けているもの | 備考 |
|---|---|---|---|
| d-1 | サイドバー脚の「レジ端末　オンライン」 | 端末の稼働情報。`kiosk_devices` に last_seen 系の列が無い | `kiosk_sessions.last_seen_at` は**レジ端末のみ**近似可（E8 staff#5 の判定材料と同一・**裁定待ち**）。打刻端末は不可（セッションを持たず `punches` に device 参照も無い） |
| d-2 | フロアの「＋ 新規伝票」（席を選ばず伝票を起こす） | `check_open` が `p_seat_id` 必須 | 席なし伝票は RPC 改稿＝DB 変更を伴う |
| d-3 | 卓タイルと会計の**定員バー**（`capacitybar` / `guestCapacity`） | `seats` に capacity 列が無い（実列＝id/name/kind/sort_order/is_active/store_id） | 卓タイル・入金モーダルの2箇所で同一原因 |
