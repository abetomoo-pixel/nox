...
## 追記（2026-08-07 キャスト・報酬レーン D2）

- ノルマ設定モック（norma.html）の「達成率別の処理」「達成ボーナス」は post-launch 送り＝受け皿となる DB（率別テーブル・ボーナス定義）が未設計で、初回はフラグ＋目標＋未達ペナルティの3実体統合まで（裁定・「準備中」表示も出さない）。
