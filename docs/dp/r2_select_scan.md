...
## 6. 走査漏れ（後から判明・SC レーンで是正）

| 箇所 | 選択肢数 | 中身 | 判定 |
|---|---|---|---|
| `app/(manage)/shift/shift-board.tsx:1184` | 4 | 下書き ／ 募集中 ／ 締切 ／ 公開済み（`PERIOD_ST_LABEL`） | **ボタン化対象（走査漏れ）** |

**理由**: R2 の全数走査は **B3 実施前**に行ったが、この期間 status の `<select>` は
**SD V2-2（commit `040930e`）で後から追加**されたため、走査時点で存在しなかった。
規約（教訓27＝固定リテラル7以下はボタン群）に照らせば**ボタン化対象**に当たる。

★同じ状態を変える口が**2つある**点も併せて是正が要る＝
B1 で計画バーに置いた「下書き保存」「スタッフに公開して確定」（`setPeriodStatus()`）と、
このフォームの status `<select>` が同じ `shift_period_set` の `p_status` を触っている。
**SC レーン（裁定43＝作成時 draft 固定）で、フォーム側は draft 固定・遷移はバー側に一本化**する。

**教訓**: 走査表は「作った時点のスナップショット」であり、**その後の実装で増えた分は自動では入らない**。
恒久規約（教訓27）を入れたなら、**新規実装のたびに規約側で弾く**（レビュー時に `<select>` を足していないか見る）
のが本筋。走査の再実行は B6 レーン（裁定48 の全画面走査）と同時に行う。
