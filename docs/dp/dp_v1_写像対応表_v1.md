...
| H3 | **対象切替「キャスト／黒服・スタッフ」** | 無し（shifts.cast_id NOT NULL→casts） | 新要件（器なし） | **要** | 下記 (1)・B4 対象外（2026-09-09）＝器なし・C層②／設計書行き |
...
| H10 | 希望提出締切 8/25 **23:59** | `shift_periods.wish_deadline date`（0101:62）・`:1285`・ブロックなし `:1341-1343` | 準備中（型不足） | 要（timestamptz 化なら） | B4 対象外（2026-09-09）＝型不足（wish_deadline は date・timestamptz 化は mig） |
...
| H15 | **勤務パターン カード**（休み希望型／出勤希望型・基本時間・固定休・役割） | **器なし**（casts は id/name/kind/employment/joined_on・役割マスタなし・`:1264-1266`「ポジション軸は実体なし」） | 新要件（器なし） | **要**（cast_work_patterns 新表） | B4 対象外（2026-09-09）＝器なし・C層②／設計書行き |
| H16 | btn スタッフ設定を開く／編集 | 無し（staff-board は権限・異動・役職のみ `:116-275`） | 新要件 | 要 | B4 対象外（2026-09-09）＝器なし・C層②／設計書行き |
