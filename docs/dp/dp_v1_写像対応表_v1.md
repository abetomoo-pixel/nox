...
| D20 | **差異理由の入力＋管理者承認（承認者・時刻を監査へ）** | 無し（note 自由文のみ・p_force は未会計専用＝差異と無関係 0055:413） | **新要件（器なし）** | **要**（diff_reason／approved_by／approved_at＋close ガード） | B2 対象外（2026-09-09）＝器なし・C層③／設計書行き |
...
| D23 | ①管理者が締め解除（理由必須） | **存在しない**（unlock RPC・ロック状態列なし） | **新要件（器なし）** | **要** | payroll_reopen（0060）が唯一の前例・B2 対象外（2026-09-09）＝器なし・C層③／設計書行き |
| D24 | ②伝票・入出金を訂正（元データ保持） | check_void 等＝audit before/after・**締め済み日の伝票編集ブロックなし** | 準備中 | 否 | enforcement 不在・B2 対象外（2026-09-09）＝器なし・C層③／設計書行き |
| D25 | ③再締め（再確定日時・担当者） | reclose は reclosed_count++ のみ（0055:497）・reclosed_at/by 列なし | 準備中 | **要**（列で持つなら） | UI `:827`・B2 対象外（2026-09-09）＝器なし・C層③／設計書行き |
...
| W32 | 訂正の監査（理由・元値・新値・操作者・承認者） | audit_log_write_service('payroll_reopen', before/after)（0060:150-156）・finalize 側も退避 | 準備中（理由・承認者なし） | 一部要 | |
