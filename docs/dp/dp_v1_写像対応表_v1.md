# デザインパック v1.0〜v1.3 写像対応表 v1（11面＝メニュー全面: 店舗設定 v3／料金 v8.1／報酬 v3.1／レジ v12.1／シフト v4.1／キャスト管理 v3.1／日報・月報・売掛 v2.1／ホーム v2.1／分析 v2.1／お知らせ・通知 v2.1／給与管理 v2.1・2026-09-04・CC・読み取り専用・裁定前）
...
| 給与管理 v2.1（W・v1.3） | 41 | 23 | 6 | 12 | 約13（半月・グループ W4-W8・既払額分離 W11・締め済み依存 W14・差分検知 W17・時給保証 W20・スタッフ給与 W24・手動調整 W26・per-cast 確定 W28-W29・次回精算 W31・明細公開 W34・振込データ W38） |
| **計（11面）** | **546** | **258** | **91** | **197** | **約134** |
...
| P12 | 入力 区分 | 無し（runPreview は p_category_id 未送信 PB:729-732） | **実装済(A層)**〔旧: 新要件（UI のみ）〕 | **不要** | ★R8=起票#52 は **M0130:137 で消化済**（pricing_resolve 6引数・whitelist 7種）＝UI 追加のみ・2026-09-07 C3 54bf807 |
| P13 | 入力 指名キャスト | 無し（`casts.rank_id` 読みの前例 PB:717-722） | **実装済(A層)**〔旧: 新要件（UI のみ）〕 | 不要 | キャスト個別料金の器なし＝実質「ランクの代理選択」・2026-09-07 裁定128＝P14 へ統合（ランクの代理選択） |
| P14 | 入力 キャストランク | 無し（p_rank_id null 固定 PB:731） | **実装済(A層)**〔旧: 新要件（UI のみ）〕 | 不要 | pricing_resolve は受理済・2026-09-07 C3 54bf807 |
| P15 | 結果「適用ルール：〜」 | liveNow 側のみ PB:855-865（R9）・プレビュー側未実装 | **実装済(A層)**〔旧: 新要件（UI のみ）〕 | 不要 | rule_id→手元 rules で name・2026-09-07 C3 54bf807 |
| P16 | 結果 セット／セット終了時刻 | 額・分は PB:1078-1084・**終了時刻は未実装** | **実装済(A層)**〔旧: 一部新要件（UI）〕 | 不要 | 入店+duration_min の表示計算・2026-09-07 C3 54bf807 |
| P17 | 結果 延長 | プレビューは set/dohan のみ（PB:733） | **実装済(A層)**〔旧: 新要件（UI のみ）〕 | 不要 | call("extension") 追加・2026-09-07 C3 54bf807 |
| P18 | 結果 本指名／場内（出所「エースランク」） | 無し | **実装済(A層)**〔旧: 新要件（UI のみ）〕 | 不要 | pricing_resolve(hon_shimei, p_rank_id)・2026-09-07 C3 54bf807 |
| P19 | 結果 VIPチャージ | 無し | **実装済(A層)**〔旧: 新要件（UI のみ）〕 | 不要 | vip_charge も whitelist 済・2026-09-07 C3 54bf807 |
| P20 | 解決順の注記 | 無し（PB:1104-1106 は別注記） | **実装済(A層)**〔旧: 新要件（注記）〕 | 不要 | 「キャスト個別」は器なし＝文言要調整・2026-09-07 C3 54bf807（注記文言＝裁定127） |
...
| P34 | VIP専用料金 使用する／しない | トグル無し＝席種 VIP のセットルールで表現（PB:1119-1122） | **実装済(A層)**〔旧: 既存（表現差）〕 | 不要 | 方式A＝席種条件／方式B＝vip_charge 併用可・2026-09-07 C2 916ac02（読み取りカードで表示） |
| P35 | VIPチャージ 使用／額／単位 | PB:1713-1722・1729・vip_charge（M0130:56）・check_open 行生成 M0130:631-649 | **実装済(A層)**〔旧: 既存〕 | 不要 | 2026-09-07 C2 916ac02（読み取りカードで表示） |
...
| P39 | VIP料金方式カード（専用／チャージ 額＋単位） | 独立カード無し（実体はルール側 P34/P35） | **実装済(A層)**〔旧: 新要件（カード器）〕 | 不要（読み取り表示なら）／要（店単位既定額列を置くなら） | 2026-09-07 C2 916ac02（読み取りのみ・裁定129） |
...
| P52 | カード手数料（日報集計用）% | `stores.card_tax_rate`（M0051:19）・PB:1397-1403 で編集・daily_report_close が列読み | **実装済(A層)**〔旧: 既存〕 | 不要 | mock は独立カード＝**移設のみ**・2026-09-07 C1 ea3b621（移設） |
| P53 | お客さまへ加算 有効／無効＋加算率 | `stores.card_surcharge_rate`（M0111:49-50）・set_store_tax_config（M0112:211）・PB:1451-1483・レジ RB:403-421（check_add_line kind='charge'） | **実装済(A層)**〔旧: 既存〕 | 不要 | ★日報用／客加算の分離は**列レベルで完成済み**・2026-09-07 C1 ea3b621（同居） |
| P54 | 転嫁の契約確認 警告文 | PB:1472-1482（警告＋**有効化時 ack チェック**＝裁定87 第2層 PB:336-338） | **実装済(A層)**〔旧: 既存〕 | 不要 | mock に ack 無し＝**残置推奨**（法務）・2026-09-07 C1 ea3b621（ack 残置＝裁定130） |
...
|---|---|---|---|---|---|
| R1 | グローバルナビ | `app/(manage)/layout.tsx:16,48`（顧客タブ＝owner/manager＋staff∧can_crm） | **実装済(A層・他面)**〔旧: 既存〕 | 不要 | 2026-09-09 変更なし（他面＝layout） |
...
| R3 | タブ [卓席・会計][予約] | `register-board.tsx:1304`・予約可視 `page.tsx:44` | **実装済(A層)**〔旧: 既存〕 | 不要 | 2026-09-09 変更なし（差分なし） |
| R4 | フロア「店舗で使う席種だけ表示」 | `page.tsx:16`（active 席全件・席種フィルタ UI なし） | 準備中（kind あり・絞込なし） | 不要 | S16/S17 のフラグと連動・2026-09-09 B3 据え置き＝S16/S17 の席種フラグと同時（裁定215） |
| R5 | 席タイル状態・合計・滞在分 | `:214,433-447,120 elapsedMin` | **実装済(A層)**〔旧: 既存〕 | 不要 | 2026-09-09 G1 d2eb1f2（滞在表記＝裁定183） |
| R6 | 席タイル 着卓キャスト顔 | `:447`・`page.tsx:35` | **実装済(A層)**〔旧: 既存〕 | 不要 | kiosk-register 非対応 `:720`・2026-09-09 変更なし（差分なし） |
| R7 | 相席「同一会計」表示 | `:30,219`（check_seats・0053） | **実装済(A層)**〔旧: 既存〕 | 不要 | 2026-09-09 変更なし（逐語一致） |
| R8 | 低在庫「残N」 | `:604-609,2396` | **実装済(A層)**〔旧: 既存〕 | 不要 | 2026-09-09 変更なし（低在庫のみ＝裁定187） |
| R9 | 開卓モーダル（人数） | `:1325-1389`・`:648`→check_open 6引数（0130:659） | **実装済(据え置き・money 境界)**〔旧: 既存〕 | 不要 | 2026-09-09 Fable 判断＝A層不触 |
| R10 | 開卓 適用ルール表示（額プレビュー） | `:1357` セットルール select・ルール名のみ＝**確定額プレビューなし** | 準備中 | 不要（pricing_resolve 呼び） | |
| R11 | 開卓 料金区分 | `:1341-1353`（0128・開栓時凍結） | **実装済(据え置き・money 境界)**〔旧: 既存〕 | 不要 | 「VIP切替」への現行回答＝変更不可・void→再開卓・2026-09-09 Fable 判断＝A層不触 |
| R12 | 開卓時に本指名を同時指定 | なし（p_nom_type free 固定 `:655`・指名は開栓後タブ `:639`） | 新要件（器＝check_set_nominations あり） | 不要 | 予約経由のみ引継（R60） |
| R13 | 卓ヘッダ（人数・滞在・合計） | `:1860,120,1199-1210` | **実装済(A層)**〔旧: 既存〕 | 不要 | 2026-09-09 G1 d2eb1f2（滞在表記＝裁定183） |
| R14 | **[卓操作] ドロワー**（6操作の集約） | 集約 UI なし（各操作は R15-R20 に散在） | 新要件（UI 集約） | 不要 | 2026-09-09 B3 据え置き＝money 隣接操作の導線再編はレジ本レーン（裁定216） |
| R15 | 卓操作① 人数変更 | `:989`→check_set_people（0090）・UI `:1860-1875` | **実装済(据え置き・money 境界)**〔旧: 既存〕 | 不要 | 0130 で set_unit 追随・2026-09-09 Fable 判断＝A層不触 |
| R16 | 卓操作② 席移動 | `:1075`→check_move_seat（0053）・UI `:1626` | **実装済(据え置き・money 境界)**〔旧: 既存〕 | 不要 | 2026-09-09 Fable 判断＝A層不触 |
| R16b | 席移動時 [現在料金を維持][VIP料金へ変更] | なし（check_move_seat に料金切替なし・区分は凍結） | 新要件（器なし） | **要**（引数追加 or check_set_category 新設） | 現行設計＝void→再開卓 `:1351` |
| R17 | 卓操作③ 相席追加（同一会計） | `:1050`→check_add_seat／`:1063` check_remove_seat（0053） | **実装済(据え置き・money 境界)**〔旧: 既存〕 | 不要 | 2026-09-09 Fable 判断＝A層不触 |
...
| R23 | 手動/自動 延長モード | time_mode・check_time_charge_apply `:928`（0052/0097） | **実装済(据え置き・money 境界)**〔旧: 既存〕 | 不要 | 2026-09-09 Fable 判断＝A層不触 |
| R24 | 「次回延長まで／次回延長 hh:mm」 | `lib/nox/check-calc.ts:30-46`・`:1885` | **実装済(据え置き・money 境界)**〔旧: 既存〕 | 不要 | 2026-09-09 Fable 判断＝A層不触 |
| R25 | 料金スナップショット表示 | `:1907`・checks 凍結列（0130 set_unit/ext_unit） | **実装済(据え置き・money 境界)**〔旧: 既存〕 | 不要 | 2026-09-09 Fable 判断＝A層不触 |
| R26 | 商品カテゴリチップ | `:390,2360-2364`（product_categories 0063/0081） | **実装済(A層)**〔旧: 既存〕 | 不要 | 2026-09-09 変更なし（「すべて」実装済） |
| R27 | 商品検索入力 | なし | 新要件（表示層） | 不要 | |
| R28 | [よく使う][最近使った] | 「最近使った」＝端末ローカル（localStorage・商品 id 最大 8）・「よく使う」は無し | **実装済(B層)**〔旧: 新要件〕 | 要（集計）or client 集計 | 2026-09-09 B3-4 2c7f357（最近使ったのみ＝裁定217・「よく使う」は集計＝対象外・新規クエリ 0） |
| R29 | 商品タイル 在庫常時表示 | stockOf（product_stock_totals RPC・既読）を常時「在庫N」・しきい以下は「残N」 | **実装済(B層)**〔旧: 準備中〕 | 不要 | 2026-09-09 B3-3 87abae2（在庫行なし＝非表示・reorder_point 未設定＝数のみ＝裁定213・タップ挙動不変・新規クエリ 0） |
| R30 | 数量ステッパー→追加 | `:414` check_add_line（連打束ね 700ms） | 準備中（表現差） | 不要 | |
| R31 | 販売実績：さくら [変更] | `:903` drink_claim_submit_proxy／`:918` drink_claim_void（0066/0067）・UI `:1391` | **実装済(据え置き・money 境界)**〔旧: 既存〕 | 不要 | back_exempt_from_split（0069）・2026-09-09 Fable 判断＝A層不触 |
| R32 | ⓘ 商品販売バックを使う店舗だけ表示 | 店フラグなし | 準備中 | 要（使うなら） | R2/S16 型のフラグ問題 |
| R33 | 帰属変更 [1名に帰属] | drink_claim_submit_proxy＝1 claim 1 cast | **実装済(据え置き・money 境界)**〔旧: 既存〕 | 不要 | 2026-09-09 Fable 判断＝A層不触 |
| R33b | **[複数キャストで按分]** | なし（drink_claims は単一 cast） | 新要件（器なし） | **要** | 指名按分（ratio_weight）とは別物 |
| R34 | 帰属モーダルの検索＋[出勤中][担当中] | cast-picker の chips prop（出勤中＝attendance／担当中＝自伝票の名簿） | **実装済(B層)**〔旧: 準備中〕 | 不要 | 2026-09-09 B3-2 e119e6c（絞込のみ・並びは名前順固定＝裁定212・新規クエリ 0） |
...
| R36 | ボトルキープ表示（残量・登録日・前回卓） | bottle_keeps 列あり・register 側では未読 | 対象外（PII ゲート後・裁定214） | 不要 | 顧客名＝**PII ゲート待ち**（can_crm）・2026-09-09 B3 対象外＝顧客名なしの先行表示はしない・前回卓は器なし・PII ゲート後に再判定（裁定214） |
...
| R39 | 商品をクリア | `:884` clearItems（drink/champ/bottle） | **実装済(据え置き・money 境界)**〔旧: 既存〕 | 不要 | 2026-09-09 Fable 判断＝A層不触 |
| R40 | 明細パネル | groupInfo `:1199`＋check-calc.ts:24（check_group_due 鏡像） | **実装済(据え置き・money 境界)**〔旧: 既存〕 | 不要 | 2026-09-09 Fable 判断＝A層不触 |
...
| R43 | キャスト追加（検索＋候補・プルダウン不使用） | cast-picker＋check_set_nominations（0119） | **実装済(A層)**〔旧: 既存〕 | 不要 | 2026-09-09 候補文＝#64 解消 B3-1 acc5be5（出勤中＝attendance／接客中＝他卓 open 名簿／打刻＝punches・ランク名は cast_ranks が読めたロールのみ＝裁定209〜211・候補一覧／帰属モーダル／指名カードの 3 面） |
| R44 | （mock に無い）指名の分配率 % | `:2029-2060`（ratio_weight・nom-shares） | **実装済(A層・他面)**〔旧: 既存（逆ギャップ）〕 | 不要 | 残置・2026-09-09 変更なし（残置） |
| R45 | 同伴（人数ステッパー・自動料金行） | `:383,680`（dohan_count） | **実装済(据え置き・money 境界)**〔旧: 既存〕 | 不要 | 2026-09-09 Fable 判断＝A層不触 |
| R46 | 請求確認（元明細→調整→サ料税→最終） | groupInfo／groupDueFull／check-calc | **実装済(据え置き・money 境界)**〔旧: 既存〕 | 不要 | 2026-09-09 Fable 判断＝A層不触 |
...
| R53 | 会計方法 現金／カード／売掛 | `:1094` check_pay・語彙4値 `:121-129` | **実装済(据え置き・money 境界)**〔旧: 既存〕 | 不要 | モックに「その他」欠落・2026-09-09 Fable 判断＝A層不触 |
| R54 | [会計を分ける]（顧客ごと／商品選択／金額指定） | pay_group（R19）＋均等割 `:1560-1570`・「顧客ごと」「金額指定」UI なし | 準備中 | 不要（group 近似） | |
| R55 | [会計を確定]＋最終確認 | `:1114` check_close・完了モーダル `:1685`（印刷 `:349`・領収書 `:1723` 0099） | **実装済(据え置き・money 境界)**〔旧: 既存〕 | 不要 | 領収書は現行の方が厚い・2026-09-09 Fable 判断＝A層不触 |
| R56 | **[会計取消・訂正]**（元伝票保持→訂正版再確定） | `:1143`→check_void（0053:441・理由必須・receivable settled 拒否） | 準備中 | 要（訂正版の再確定を作るなら） | void は取消のみ＝訂正版生成・紐付けなし |
| R57 | 会計後タイムライン | audit_logs にはある・レジ画面の履歴 UI なし | 準備中 | 不要 |・2026-09-09 停止リスト継続（裁定189・新規 UI＝レジ本レーンでも A層外） |
| R58 | 予約リスト | `reservation-panel.tsx:172`（reservations 列群） | **実装済(A層・他面)**〔旧: 既存〕 | 不要 | 電話表示 `:437`＝**PII ゲート待ち**・2026-09-09 変更なし（他面・電話は PII） |
| R59 | [＋予約を追加]／編集／取消 | reservation_create／update／set_status（0027/0029） | **実装済(A層・他面)**〔旧: 既存〕 | 不要 | 定休日ブロック（0032）・2026-09-09 変更なし（他面） |
...
|---|---|---|---|---|---|
| H1 | サブナビ5タブ | `shift-board.tsx:687-688`（today/queue/build/calendar/roster） | **実装済(A層)**〔旧: 既存〕 | 否 | 語順一致・2026-09-07 S1 fa4d613（差分なし） |
| H2 | 承認待ちバッジ ④ | `:1149`（件数ピル） | **実装済(A層)**〔旧: 既存〕 | 否 | タブ側バッジは小差・2026-09-07 S1 fa4d613（丸数字） |
| H3 | **対象切替「キャスト／黒服・スタッフ」** | shift-board の対象切替（flag_enabled('staff_shift') 自店＝true のときだけ）→ staff-shift-board（本人 ◯×）／staff-shift-manage（配置・上書き・確定） | **実装済(C層②)**〔旧: 新要件（器なし）〕 | 0136／0137 で解消 | 2026-09-09 C層② c742261（切替）／ae7ee97（面 b 本人）／946e643（面 c manager）／d2bf48e（面 a 店舗設定＝/master/business-hours 下の黒服シフト節）・flag off＝切替不在＝現行そのまま・黒服は別器（staff_shifts 4 表）で cast の shifts は不触 |
| H4 | 期間粒度 月/半月/週＋表示期間切替 | 月のみ（`:945-947`） | 新要件（UI のみ） | 否 | 2026-09-09 B4 据え置き＝shift_periods の粒度運用と同時（裁定227） |
| H5 | フロー帯 申請→承認→作成→公開→仮→確定 | 4段版＝承認待ちタブ `:1116-1167` | **実装済(A層)**〔旧: 既存（語彙差）〕 | 否 | 「公開」は period 側・2026-09-07 S1 fa4d613（4段語彙＝裁定133） |
| H6 | 見出し「2026年9月 キャストシフト計画」 | `:1282` | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 S1 fa4d613 |
| H7 | btn 必要人数を設定 | 仮シフトタブ内カード `:1036-1095`（set_staffing_need／staffing_need_remove `:483,:495`） | **実装済(A層)**〔旧: 既存（配置差）〕 | 否 | モックはモーダル・2026-09-07 S2 085e8d3（スクロール導線＝裁定136） |
| H8 | btn スタッフへ公開 | `:1294-1296`→shift_period_set(status='published')（`:420`） | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし＝裁定134 |
| H9 | 期間バッジ「作成中」 | PERIOD_ST_LABEL `:65`・`:1289`（draft=下書き） | **実装済(A層)**〔旧: 既存（語彙差）〕 | 否 | 2026-09-07 S1 fa4d613（作成中＝裁定135） |
| H10 | 希望提出締切 8/25 **23:59** | `shift_periods.wish_deadline date`（0101:62）・`:1285`・ブロックなし `:1341-1343` | 準備中（型不足） | 要（timestamptz 化なら） | B4 対象外（2026-09-09）＝型不足（wish_deadline は date・timestamptz 化は mig）・C層②（2026-09-09）＝黒服側の締切は staff_shift_deadlines（N 日前＋時刻・effective_from 型）で実装。cast 側の wish_deadline（date）は不触＝C②-8 の非対称 |
| H11 | 仮シフト公開予定／確定予定（日時） | 列なし | 新要件（器なし） | **要**（publish_planned_at／confirm_planned_at） | B4 対象外（2026-09-09）＝器なし・C層②／設計書行き |
| H12 | KPI 配置済み／予想人件費／不足日／未処理希望 | `:1307-1312`・月サマリ `:966-971`・`lib/nox/labor-forecast.ts` | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 S1 fa4d613（不足日） |
| H13 | 月カレンダー＋日クリック→不足と追加候補 | `:1369-1400`／`:975-1000`→日モーダル `:1625/:1777` | **実装済(A層)**〔旧: 既存〕 | 否 | 裁定121 経路・2026-09-07 変更なし（差分なし・裁定121） |
| H14 | 凡例 🔴🟡🟢＋注記 | `:1008-1015`・`:1653-1655` | **実装済(A層)**〔旧: 既存〕 | 否 | 現行は余剰＝灰も・2026-09-07 変更なし（現行4色残置） |
| H15 | **勤務パターン カード**（休み希望型／出勤希望型・基本時間・固定休・役割） | **器なし**（casts は id/name/kind/employment/joined_on・役割マスタなし・`:1264-1266`「ポジション軸は実体なし」） | 新要件（器なし） | **要**（cast_work_patterns 新表） | B4 対象外（2026-09-09）＝器なし・C層②／設計書行き・C層②（2026-09-09）＝勤務パターンは「店舗定義の枠（名前・開始・終了・effective_from）」として別器で実装（面 a）。モックの個人条件型（休み希望型／出勤希望型・固定休・役割）は C②-1 で採らず＝この行は据え置き |
| H16 | btn スタッフ設定を開く／編集 | 無し（staff-board は権限・異動・役職のみ `:116-275`） | 新要件 | 要 | B4 対象外（2026-09-09）＝器なし・C層②／設計書行き・C層②（2026-09-09）＝枠と締切の設定は /master/business-hours の「黒服シフト」節（面 a）。「スタッフ設定を開く」導線はモックの個人条件型を前提＝据え置き |
| H17 | 通知設定3種（公開時／変更時／確定時） | **無し**（notify/LINE 実装 0 件） | 新要件（器なし） | **要** | 下記 (5)・B4 対象外（2026-09-09）＝器なし・C層②／設計書行き |
| H18 | 承認待ち KPI（未処理／申請者／希望どおり／要調整） | wishes（pending）＋wish_id を持つ planned／proposed の時刻一致で導出 | **実装済(B層)**〔旧: 準備中〕 | 否 | 2026-09-09 B4-b 180fe0d（未処理／申請者／希望どおり／要調整＝裁定218・新規クエリ 0） |
| H19 | btn 選択分を一括承認 | 一括＝「まとめてキャスト確認へ」`:1134-1138`（shift_propose）と「一括確定」`:1141-1148`（shift_confirm_bulk 0126）・**wish の一括 accept RPC は無い**（shift_wish_decide は1件） | **実装済(B層)**〔旧: 準備中〕 | 否（ループ発行）/ 要（一括 RPC なら） | 2026-09-09 N4 daee5e2（ループ発行）＝裁定197 区分訂正 |
| H20 | 人ごとグルーピング | 承認表の並び切替（日付順／人ごと・人ごとは名前順に見出し行） | **実装済(B層)**〔旧: 新要件（UI のみ）〕 | 否 | 2026-09-09 B4-b 180fe0d（既定は現行の日付順＝裁定219） |
| H21 | 差分「本人希望→管理側」 | `:1202-1207`・日詳細 `:1741-1745` | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 S1 fa4d613（本人希望→管理側＝裁定137） |
| H22 | btn 希望どおり承認 | `:1220`→decide→shift_wish_decide(true)（0103:287-334・planned 自動生成） | **実装済(A層)**〔旧: 既存〕 | 否 | 下記 (3)・2026-09-07 S1 fa4d613（希望どおり） |
| H23 | btn 時間調整（承認時） | wish 段には**意図的に非表示**（`:1109-1110`）・planned 段の時間調整 `:1227`→shift_set 据置 `:387`・ウィザードのみ accept→shift_set の合成（`shift-add-form.tsx:268-278`） | 準備中 | 否（UI 合成）/ 要（shift_wish_decide_with_time 新設なら） | 下記 (3)・B4 対象外（2026-09-09）＝書込合成（shift_wish_decide→shift_set の 2 段）は RPC 新設と同時に裁定 |
| H24 | btn 見送り | `:1221`→shift_wish_decide(false)（定休日でも可 0103:306-308） | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
| H25 | **休み希望**（休み承認／却下・現在予定との対比） | shift_wishes に種別なし・近縁＝cast_unavailable_days（owner/manager 起票のみ・cast セルフは起票#49） | 新要件（器なし） | **要**（shift_wishes.kind 'work'/'off' or 申請表） | B4 対象外（2026-09-09）＝器なし・C層②／設計書行き |
| H26 | 仮シフト「公開済み 9/1 18:00」「変更内容を再公開」「公開後変更5件」 | published はあるが**公開日時・再公開・公開後差分の器なし** | 新要件（器なし） | **要**（published_at＋公開スナップショット） | B4 対象外（2026-09-09）＝器なし・C層②／設計書行き |
| H27 | 未確認／確認済み／変更希望／辞退希望 | shifts.status（proposed＝未確認／confirmed＝確認済み）の人別再形 | **実装済(B層)**〔旧: 新要件（一部既存）〕 | 要 | 2026-09-09 B4-c 436840b（確認済み／未確認の 2 値のみ＝裁定226・変更希望・辞退希望・催促は器なし） |
| H28 | 行操作 詳細／確認／催促／対応 | 無し（催促＝通知手段なし） | 新要件 | 要 | H17 と同根・B4 対象外（2026-09-09）＝器なし・C層②／設計書行き |
| H29 | 公開状態カード＋「確定シフトへ」 | `:1289`＋setTab("roster") | **実装済(A層)**〔旧: 既存（部分）〕 | 否 | 2026-09-07 S2 085e8d3（確定シフトへ＝裁定138） |
| H30 | 公開後の変更（時間変更／追加／取消） | audit_logs（shift_set 0125:207・bulk_daily 0125:290・wish_decide 0103:330・confirm_bulk 0126:60）・**シフト UI に履歴表示なし**・shift_remove（0103:467）は **UI 未結線** | **実装済(B層・shift_remove 結線)**〔旧: 準備中（器あり・UI なし）〕 | 否 | 下記 (2)・2026-09-09 N4 daee5e2＝裁定197 区分訂正・履歴表示は裁定190（owner のみ）・2026-09-09 B4 変更なし（実装済 B層のまま） |
| H31 | 今日 KPI（出勤予定／出勤済み／遅刻未着／欠勤） | shiftsOn＋attendance 5 値＋punches の再形＝4 カウンタ（本日の勤務表の上） | **実装済(B層)**〔旧: 準備中（軸違い）〕 | 否 | 2026-09-09 B4-a 15b52ef（出勤予定／出勤済み／遅刻・未着／欠勤＝裁定220・221・既存 KPI 帯と予想人件費は不触） |
| H32 | 本日の勤務「20:01打刻」「未着 20:18現在」 | punches（表示日の営業日範囲・自店）→ 最終 'in' を「HH:MM打刻」 | **実装済(B層)**〔旧: 準備中（器あり・UI なし）〕 | 否 | 2026-09-09 B4-a 15b52ef（出勤記録セルの補助表示・書込なし＝裁定222・新規クエリ 1） |
| H33 | btn ＋当日追加配置 | `:809-818`（当日は confirmed 既定＝裁定42） | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 S1 fa4d613（＋当日追加配置） |
| H34 | 現在の不足（時間帯別）＋追加配置 | BandBars `:908-931`・dayStat `:571-581` | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし |
| H35 | 本日の通知（遅刻超過／欠勤→代打手配） | 無し | 新要件（器なし） | 要 | H17 と同根・B4 対象外（2026-09-09）＝器なし・C層②／設計書行き |
| H36 | btn 連絡 | 無し | 新要件 | 要 | B4 対象外（2026-09-09）＝器なし・C層②／設計書行き |
| H37 | 確定シフト PDF/印刷 | 印刷ボタン＝window.print＋.nox-printpage 隔離（確定シフトタブ・表のみ）・CSV は残す | **実装済(B層)**〔旧: 新要件（UI のみ）〕 | 否 | 2026-09-09 B4-c 436840b（PDF はブラウザ印刷＝裁定223） |
| H38 | 期間別ステータス「9/1〜9/15 確定済み 確定 8/29 18:20」 | shift_periods 一覧 `:1317-1327`・**確定日時列なし・status に confirmed 値なし** | 準備中 | **要** | 期間単位確定は現行では shift 行単位（confirm_bulk）・B4 対象外（2026-09-09）＝器なし・C層②／設計書行き |
| H39 | 確定後の変更履歴＋「管理者変更を追加」 | audit_logs（表示月・自店・action like 'shift%'・最新 60 件）を owner のみ読取→変更履歴 | **実装済(B層)**〔旧: 準備中（器あり・UI なし）〕 | 否 | 2026-09-09 B4-c 436840b（owner のみ描画・manager 非表示＝裁定224・「管理者変更を追加」は器なし＝出さない・新規クエリ 1） |
| H40 | 日モーダル「9月4日（金）の配置 必要5名／配置2名」 | `:1777-1819`／`:1625-1707`・充足ピル `:1634-1636` | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（充足ピル維持） |
| H41 | 候補検索（名前）＋勤務条件を候補カードに反映 | DayAddPanel に名前部分一致の検索 input（並びは名前昇順のまま） | **実装済(B層)**〔旧: 新要件（一部既存）〕 | 要（条件表示）/ 否（検索） | 2026-09-09 B4-b 180fe0d（検索のみ＝裁定225・勤務条件の表示は H15 後） |
| H42 | btn 選択した1名を配置 | `day-add-panel.tsx:91-111`（行ごと shift_set(planned)・部分成功維持） | **実装済(A層)**〔旧: 既存〕 | 否 | 現行は複数行バッファ＝挙動優位・2026-09-07 S1 fa4d613（選択したN名を配置・挙動不変） |
| H43 | 必要人数モーダル（曜日基本＋時間帯上乗せ） | `:1071-1094`＋set_staffing_need(店,dow,required,from_min,to_min)（0095:47） | **実装済(A層)**〔旧: 既存（形状差）〕 | 否 | 「上乗せ」でなくバンド絶対値・2026-09-07 変更なし（バンド絶対値維持＝裁定136） |
| H44 | **代打・交代を手配**（欠勤者→候補・フィルタ4種・依頼通知・承諾後反映・即時確定） | **器なし**（1日1枠制約で欠勤者行を残して代打を足せない・依頼→承諾のワークフロー列なし） | 新要件（器なし） | **要**（shift_substitutions 新表） | 下記 (2)・B4 対象外（2026-09-09）＝器なし・C層②／設計書行き |
...
| # | v3.1 要素 | 現行実装 | 分類 | migration | 備考 |
|---|---|---|---|---|---|
| K1 | 左ナビ（キャスト／スタッフ別項目） | `app/(manage)/casts/page.tsx:11`・`staff/page.tsx` | **実装済(A層)**〔旧: 既存〕 | 否 | モックは同一画面のセグメント切替・2026-09-07 変更なし（別ページ構成） |
| K2 | ヘッダ（CAST MANAGEMENT／lead） | `casts-board.tsx:367-368` | **実装済(A層)**〔旧: 既存〕 | 否 | 文言差・2026-09-07 C1 0b896ce（lead＝裁定141） |
| K3 | セグメント「キャスト／黒服・スタッフ」1画面統合 | 現行は別ページ（casts-board／staff-board） | 新要件（UI 統合） | 否 | |
| K4 | KPI 在籍 | `casts-board.tsx:355`（is_active） | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
| K5 | KPI 時給保証中 | なし（現行は「体入中」`:374`＝trials） | 新要件（器なし） | 要 | K33 前提 |
| K6 | KPI LINE未連携 | なし | 新要件（器なし） | 要 | K37 前提 |
| K7 | KPI メール未登録 | なし（「未招待」`:356`＝user_id null で近似） | **実装済(B層)**〔旧: 準備中〕 | 否〜要 | users.email は一覧未取得 `page.tsx:28`・2026-09-09 B1-b 777fe75（users.email 1 クエリ＝裁定193） |
| K8 | 名前検索 | `:346 hit()`／`:381` | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
| K9 | フィルタ 在籍／休入／退店済み | `:386` seg＝active／trial（体入）／left | **実装済(A層)**〔旧: 既存（語彙差）〕 | 否 | 「休入」＝体入相当・現行は is_active＋trials・2026-09-07 変更なし（体入据え置き＝裁定140） |
...
| K13 | カード アバター | `:501` CastAvatar＋photoUrls | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
| K14 | カード 名前 | `:503` | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
| K15 | カード meta「在籍／ログイン済み」 | `:504`（is_active＋user_id） | **実装済(A層)**〔旧: 既存〕 | 否 | 逐語一致・2026-09-07 変更なし（逐語一致） |
| K16 | カードバッジ レギュラー／エース／体験入店 | `:506-510`（rank名／plan名の金色行） | **実装済(B層)**〔旧: 準備中〕 | 否 | cast_ranks 流用・2026-09-09 B1-b 777fe75（nox-stpill 2 バッジ） |
...
| K21 | 詳細ヘッダ（写真・名前・meta・閉じる） | `:524-541` | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
...
| K24 | プロフィール写真（アップロード／**トリミング**／**削除**／D&D） | `:866-898` モーダル＝アップロード（上書き）のみ・`lib/nox/cast-photo.ts:15` bucket cast-photos `{org_id}/{cast_id}.jpg`・set_cast_photo_updated_at（0065）・policy 3本は手貼り（手貼りリスト:118-204）・`:865`「削除経路は持たない」・本人側 `app/mine/photo-card.tsx` | **実装済(A層)**〔旧: 既存（一部新）〕 | 否（トリミングは client）／要 policy（削除＝delete policy なし） | 複数枚不可（1cast1ファイル）・2026-09-07 C1 0b896ce（文言のみ＝裁定144） |
...
| K26 | 基本情報 源氏名／氏名 | `:555-577` set_cast_profile（0122） | **実装済(A層)**〔旧: 既存〕 | 否 | ★氏名（本名）並記は **PII ゲート待ち**（owner 限定＋audit 経路）・2026-09-07 変更なし（本名並記は PII ゲート待ち） |
| K27 | 基本情報 本入店日 | `:563` joined_on（0074） | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 C1 0b896ce（本入店日＝裁定142） |
...
| K34 | 待遇・時給の適用履歴 | 器あり＝cast_plan 期間化（0114・valid_from/valid_to・部分 unique 2本・set_cast_plan p_valid_from）・**読む側は現在行のみ**（`comp-sections.tsx:159`）＝履歴 UI なし・先例＝`deduction-board.tsx:6-7`「変更履歴は実装しない＝裁定6・実体は audit」 | **実装済(B層)**〔旧: 準備中（器あり・UI なし）〕 | 否 | 在籍履歴（K25）は器ごと無し＝別扱い・2026-09-09 N4 daee5e2（履歴 ▸）＝裁定197 区分訂正 |
| K35 | 通常の待遇・バック（プラン／通常時給／報酬ルール） | `:649-687` 表示専用・編集は AssignTab（`comp-sections.tsx:428`） | **実装済(A層)**〔旧: 既存〕 | 否 | モックは詳細内編集・2026-09-07 C1 0b896ce（通常時給（プラン基本）＝裁定143） |
| K36 | 連絡・アカウント: 本人レコード／NOX ログインの分離説明 | `:691-716`（招待状態・PIN）＋`:714` | **実装済(A層)**〔旧: 既存〕 | 否 | 分離思想一致・2026-09-07 C1 0b896ce（2カード語彙） |
...
| K40 | （mock に無い）打刻 PIN | `:704-712` set_cast_pin（0043）・kiosk-pin-panel | **実装済(A層)**〔旧: 既存（**モック側欠落**）〕 | 否 | 統合時に消さない・2026-09-07 変更なし（残置） |
| K41 | （mock に無い）体入 trials 詳細（評価／書類4種／本採用／見送り） | `:723-770` trial_update／hire／reject | **実装済(A層)**〔旧: 既存（**モック側欠落**）〕 | 否 | モック「体験入店」ステータスと二重管理の論点・2026-09-07 変更なし（残置・見送り＝Danger 系＝裁定146） |
...
| K44 | スタッフ一覧（役職／在籍／メール／店） | `staff-board.tsx:189-225`（memberships＋users） | **実装済(A層)**〔旧: 既存〕 | 否 | 行→カード・2026-09-07 変更なし（表形式維持） |
| K45 | スタッフ権限（会計／顧客／シフト／バック） | `:38-43` PERM_DEFS→set_staff_perms（`:116`）＝boolean 4フラグ | **実装済(A層)**〔旧: 既存〕 | 否 | ★モック「閲覧のみ／編集可／不可」3値 vs 現行2値＝粒度差・2026-09-07 変更なし（2値のまま＝裁定145） |
| K46 | スタッフ役割（主担当／対応可能役割／配置優先） | なし（memberships.role は owner/manager/staff/cast） | 新要件（器なし） | 要 | ＝H15 スタッフ側 |
| K47 | スタッフ役職変更 | `:258-265` staff_change_role | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
| K48 | スタッフ異動 | `:244-256` staff_transfer_store（0025・owner） | **実装済(A層)**〔旧: 既存〕 | 否 | キャスト側は無し（K28）・2026-09-07 変更なし（差分なし） |
| K49 | スタッフ在籍解除／再雇用 | `:266-278` staff_deactivate／reactivate | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（在籍解除＝Danger 系＝裁定146） |
| K50 | スタッフ追加モーダル | `:285-335` POST /api/staff/create | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
...
31. **給与の確定粒度（W28/W29）＝既裁定「run 単位・キャスト単位確定は作らない」（U1 設計書:17）との正面衝突**: 採るなら裁定見直し＋payslip 単位 status＋部分 finalize＋idem＋reopen 対称化＝money-core 大改修（Fable・設計書→f0）。実装順は **W26 手動調整→W28 per-cast 確定→W29 中間状態→W31 次回精算**の依存順。
32. **手動調整（W26/W31）＝裁定99-⑥「採用しない・準備中表示もしない」の見直し**: payroll_adjustments（run×cast・符号付・reason・created_by・approved_by）＋finalize 取込＋reopen 巻き戻し。Downloads の `0138_wage_adjustments_rpc.sql` は未収蔵＝別系統の下書き（照合・裁定なしに取り込まない）。
33. **確定後の訂正（W30/W32）＝D-1 同族＝同レーン**: payroll_reopen は run 全体・一部支払済みで全員分不可・理由/承認者列なし＝日報の締め解除（要点23）と同じ形（unlock＋理由＋承認者）へ揃える裁定。
34. **給与の期間・グループ（W4-W8）＝money-core 最深部の別レーン**: 半月／週サイクル・支払予定日・給与グループ（キャスト／正社員／日払い）は period_bounds・runs unique・payslips.period・源泉 periodDays（裁定23）の全面改修＝設計書必須。
35. **既払額の会計分離（W11）・期間付き時給保証（W20＝K33）・スタッフ給与（W24＝H3/K51 の二軸）**: 賃金定義に触れる＝**S-1（社労士）系ゲート**。振込データ（W38）は口座＝機微＝**専門家ゲート待ち**。源泉納付（W39）の順延・母数＝**税理士ゲート待ち**（裁定23／T1-T3）＋`payroll_mark_paid` を呼ぶ UI 不在（運用穴＝単独起票候補）。
36. **集計元と締め済み依存（W14）**: 現行は daily_reports 非依存（checks closed・shifts confirmed）＝モックの「締め済み日報のみ／未締め日があると計算不可」は新定義＝日報 v2.1（X2・要点23/24）と一体で裁定。
...
|---|---|---|---|---|---|
| D1 | 画面ヘッダ「日報・締め管理」 | `report-board.tsx:275-276` | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 R1 98fb7d0（lead＝裁定147） |
| D2 | タブ 日報／月報／売掛 | `:282-290`（ar は isManagerUp） | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
| D3 | 「営業中・未締め」バッジ | `:423-433`（daily_reports 行の有無） | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 R1 98fb7d0（営業中・未締め＝裁定150） |
| D4 | 「締め確認へ」→締め確認モーダル | 無し（締めは同画面下部フォーム `:726-738`） | 新要件（UI） | 否 | |
| D5 | 営業日表示 | `:566-568` | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
| D6 | 営業時間 19:00〜翌5:00 | `store_business_hours`（0032:41-42）を report-board で読取（dow 7 行・表示のみ） | **実装済(B層)**〔旧: 新要件（器あり・UI なし）〕 | 否 | 2026-09-09 B2-a 02f0033（営業日切替の隣に「営業時間 HH:MM〜翌HH:MM」・定休日は「定休日」・行なし店は非表示・cast 非到達＝裁定205） |
| D7 | 営業日切替 翌06:00 | settings_json.biz_cutoff_hm・set_store_biz_cutoff（0106） | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 R1 98fb7d0（営業日切替 翌HH:MM＝裁定150） |
| D8 | 「現在 9/5 02:14→9/4 営業日」 | bizDateOf（初期値算出＋ヘッダ表示） | **実装済(B層)**〔旧: 新要件（UI）〕 | 否 | 2026-09-09 B2-a 02f0033（「現在 HH:MM → MM/DD 営業日」＝純関数・1 分更新） |
| D9 | KPI 確定売上／組数／客単価／現金／カード等 | `:445-479`（5枚・前日比） | **実装済(A層)**〔旧: 既存〕 | 否 | 現行は「暫定」表記・2026-09-07 R1 98fb7d0（並びのみ・暫定据え置き＝裁定148） |
| D10 | 締めフロー 01〜05 ステップ | 無し（チェック3項目 `:676-703`） | 新要件（UI） | 否 | 既存値の再形 |
| D11 | 未会計伝票 0件 | `:682` preview.open／close ガード `open checks remain`（0055:413） | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
| D12 | 取消・返金（監査ログあり） | audit_logs の取消系 action（audit-board「取消・巻き戻し」と同じ明示リスト）を owner のみ count | **実装済(B層)**〔旧: 新要件（器あり・UI なし）〕 | 否 | 2026-09-09 B2-a 02f0033（owner＝件数＋「操作履歴を見る」導線／manager 以下は非描画・RLS owner 限定のまま＝裁定202） |
| D13 | 実査現金 未入力／入力 | `:684-686,730` | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
| D14 | 売掛 本日発生 2件／¥ | receivables.created_at を営業日範囲で select（件数＋金額） | **実装済(B層)**〔旧: 準備中〕 | 否 | 2026-09-09 B2-a 02f0033（「発生」＝created_at を bizDateOf の cutoff で営業日正規化＝裁定204・manager 以上のみ） |
| D15 | 未処理の入出金（日払い・送り代・立替） | advances／transport の status='open' を自店で select（件数＋金額のみ） | **実装済(B層)**〔旧: 新要件（器あり・UI なし）〕 | 否 | 2026-09-09 B2-a 02f0033（日払い（前借り）／送り代の未処理 2 枚・立替は器なし＝「対象外」注記・cast 名不出＝裁定203） |
| D16 | 諸経費・現金支払「登録済み分」 | 諸経費テーブルなし＝締め時 int 入力のみ（expense／cash_payout 0010:81-82） | 新要件（器なし） | **要**（明細表が要るなら） | B2 対象外（2026-09-09）＝器なし・C層③／設計書行き |
| D17 | 現金照合 レジ内予定額 | `:631`（cashFloat+cash+arCollectedToday−expense−payout＝DB 式 0055:419-421 と一致） | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし・式不変） |
| D18 | 実査現金／釣銭準備金／実査差異 | counted_cash／cash_float／diff（0010:82-85）・UI `:635-652` | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 R1 98fb7d0（実査差異＝裁定150） |
| D19 | 「実査を入力」 | `:730`＋金種カウンタ `:744-776` | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし |
| D20 | **差異理由の入力＋管理者承認（承認者・時刻を監査へ）** | 無し（note 自由文のみ・p_force は未会計専用＝差異と無関係 0055:413） | **実装済(C層③)**〔旧: 新要件（器なし）〕 | **要**（diff_reason／approved_by／approved_at＋close ガード） | B2 対象外（2026-09-09）＝器なし・C層③／設計書行き ・2026-09-10 **実装(C層③ 面a a801683)**: /report 締め済み行「差異を承認」（理由必須・cash_diff_approve mig0138）→承認済み（理由／承認者／日時）。flag off＝導線不在 |
| D21 | 売上内訳5分類 | `:496-500` kindSums（時間／ドリンク／シャンパン／ボトル／その他） | **実装済(A層)**〔旧: 既存（分類粒度差）〕 | 否 | 再マップ要・2026-09-07 R2 9531d41（シャンパン・ボトル表示合算＝裁定149） |
| D22 | 締め履歴の説明「締め解除→訂正→再確定」 | `daily_report_reclose`（0055:450）＝**解除なしの直接再集計** | 準備中 | 否 | 下記 (1)・2026-09-09 裁定208＝据え置き（説明文は現行仕様「再集計」のまま・C層③後に差替） |
| D23 | ①管理者が締め解除（理由必須） | **存在しない**（unlock RPC・ロック状態列なし） | **実装済(C層③)**〔旧: 新要件（器なし）〕 | **要** | payroll_reopen（0060）が唯一の前例・B2 対象外（2026-09-09）＝器なし・C層③／設計書行き ・2026-09-10 **実装(C層③ 面a a801683)**: /report 締め済み行「解除」（理由必須モーダル・report_reopen mig0138）→解除中バッジ→「再締め」（daily_report_reclose p_idem_key）。staff は can_reopen／can_close で出し分け。flag off＝導線不在 |
| D24 | ②伝票・入出金を訂正（元データ保持） | check_void 等＝audit before/after・**締め済み日の伝票編集ブロックなし** | **実装済(C層③)**〔旧: 準備中〕 | 否 | enforcement 不在・B2 対象外（2026-09-09）＝器なし・C層③／設計書行き ・2026-09-10 **実装(C層③ 面b 3e8d0cb)**: 関所 assert_day_open（mig0140／0141＝書込 RPC 16 本）＋/register の先回り無効化（バナー・商品タップ・money 系 17 ハンドラ）・day closed 写像。解除中は操作可 |
| D25 | ③再締め（再確定日時・担当者） | reclose は reclosed_count++ のみ（0055:497）・reclosed_at/by 列なし | **実装済(C層③)**〔旧: 準備中〕 | **要**（列で持つなら） | UI `:827`・B2 対象外（2026-09-09）＝器なし・C層③／設計書行き ・2026-09-10 **実装(C層③ 面a a801683)**: reclosed_at／reclosed_by／reclose_idem_key（mig0138）＝再締めは解除中のみ・冪等。/report は解除中バッジ＋再締め者名（memberships→users） |
| D26 | 締め履歴リスト | `:783-795`（closed_by→users.name） | **実装済(A層)**〔旧: 既存〕 | 否 | closed_at 非表示・2026-09-07 変更なし |
| D27 | 月報 対象月 | `month-report.tsx:159`＋period_bounds | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
| D28 | 前半／後半 | `month-report.tsx:53,61`（裁定12②） | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
| D29 | 月報 CSV出力 | 無し＝**裁定12⑥「表示のみ・CSV なし」** | 新要件（裁定変更要） | 否 | B2 対象外（2026-09-09）＝裁定12⑥の変更要 |
| D30 | **月次確定ルール（未締め日があると確定不可）** | 無し＝**裁定12①「月次確定テーブルは作らない」** | **新要件（器なし）** | **要**（確定を持つなら） | 下記 (4)・B2 対象外（2026-09-09）＝器なし・C層③／設計書行き |
| D31 | 前半「確定3日／未確定12日」 | 無し（締め済み日数のみ・母数を知らない） | 新要件 | 否〜要 | 母数＝store_business_hours・B2 対象外（2026-09-09）＝母数（営業日数）の裁定要 |
| D32 | 月報 KPI | `month-report.tsx:164-185`＋人件費率 `:143` | **実装済(A層)**〔旧: 既存（構成差）〕 | 否 | 2026-09-07 変更なし（構成差は残置） |
| D33 | 月報表（前半／後半／通期） | `month-report.tsx:132-144`（人件費は通期のみ＝台帳:262） | **実装済(A層)**〔旧: 既存〕 | 否 | モックも通期のみ＝一致・2026-09-07 変更なし（差分なし） |
| D34 | 日別確定状況（締め済み／営業中） | `month-report.tsx:66-68,216-236` | **実装済(B層)**〔旧: 準備中〕 | 否 | 「営業中」＝行なしの表現・2026-09-09 month-report 描画済み＝裁定197 区分訂正 |
| D35 | 店舗設定で売掛 OFF→全非表示 | settings_json に AR トグルなし | **新要件（器なし）** | **要** | ＝S24（receivable_policy='disabled' で写せる）・B2 対象外（2026-09-09）＝器なし・C層③／設計書行き |
| D36 | 売掛 KPI（残高／件数／期限超過／今月回収） | `:296-311`（残高式 0092 準拠） | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし |
| D37 | KPI 今月発生 | receivables.created_at を営業日月の範囲で select（件数＋金額） | **実装済(B層)**〔旧: 準備中〕 | 否 | 2026-09-09 B2-b d5e7142（KPI 5 枚目「今月発生」＝裁定204） |
| D38 | 「＋売掛を登録」（手動作成） | 手動 RPC なし＝発生は check_pay(ar)→receivables INSERT（0007:362）のみ | 新要件（器なし） | **要** | money-core 非接触方針と衝突＝要検討・B2 対象外（2026-09-09）＝器なし・C層③／設計書行き |
| D39 | 顧客名・伝票番号検索 | 無し（並び替えのみ `:312-316`） | 新要件 | 否 | **顧客は PII ゲート待ち**・B2 対象外（2026-09-09）＝PII ゲート待ち |
| D40 | フィルタ 期限超過／今月発生／一部回収 | client フィルタ（due／created_at／collected_amount） | **実装済(B層)**〔旧: 準備中〕 | 否 | 2026-09-09 B2-b d5e7142（すべて／期限超過／今月発生／一部回収・既存 select へ created_at 列追加のみ・KPI は全件のまま） |
| D41 | 未回収行（顧客名・伝票#・担当・残/回収済・期限） | `:321-345`（伝票番号・担当は未表示）・customers.name は owner/manager/staff∧can_register へ開示中（0055:143-150） | **実装済(A層)**〔旧: 既存〕 | 否 | **顧客は PII ゲート待ち**（範囲拡大分）・2026-09-07 変更なし（伝票番号は残置・PII ゲート待ち） |
| D42 | 回収方法／前回／入金日 | ar_collections.method/biz_date/amount を open 行ごとに最新 1 件で読取 | **実装済(B層)**〔旧: 新要件（器あり・UI なし）〕 | 否 | 2026-09-09 B2-b d5e7142（未回収行に「前回回収 入金日・方法・金額」を併記・表示のみ） |
| D43 | 伝票を見る／履歴 | 無し（receivables.check_id あり） | 新要件（UI） | 否 | 2026-09-09 裁定208＝据え置き（R57 停止中＝裁定189 同型） |
| D44 | 回収を登録／追加回収（部分） | receivable_collect（0092 6引数）・UI `:379-400` | **実装済(A層)**〔旧: 既存〕 | 否 | verify-nox-ar-partial・2026-09-07 R1 98fb7d0（回収を登録／追加回収＝裁定150） |
| D45 | 回収方法の選択（現金／カード／その他） | RPC は3値受理（0055:96,194）・**UI は "cash" 固定** `:238`・ar_collected は cash のみ（0055:351-353） | 準備中(money 境界) | 否 | select 追加で開通・2026-09-09 裁定206＝据え置き（C層③で入金方法別照合と同時・UI は cash 固定のまま） |
| D46 | 給与天引き経路 | receivable_mark_deduct（0055:251・同意）・payroll_finalize | **実装済(A層)**〔旧: 既存〕 | 否 | モックに天引き記載なし＝現行が上位互換・2026-09-07 変更なし（残置） |
| D47 | 回収履歴リスト | ar_collections 一覧（当月・自店・created_at 降順 30 件・customers／casts／users 埋込） | **実装済(B層)**〔旧: 新要件（器あり・UI なし）〕 | 否 | 2026-09-09 B2-b d5e7142（列＝入金日／方法／金額／顧客／担当／登録者・顧客は未回収一覧と同粒度・「それ以前は…」導線文＝裁定207） |
| D48 | 「partial／oldest-first ルールは設定側」 | oldest-first 自動充当なし | 新要件（器なし） | 要（設定を持つなら） | B2 対象外（2026-09-09）＝器なし・C層③／設計書行き |
| D49 | 締め確認モーダル（3チェック＋不可メッセージ） | ロジックは `:676-703` に存在（位置差） | **実装済(B層)**〔旧: 準備中〕 | 否 | 2026-09-09 B2-a 02f0033（3 項目を「締め確認」節として締め直上へ・未会計あり＋強行オフ時の不可メッセージ・判定式不変＝裁定196・差異承認は D20 依存のまま） |
...
| # | v2.1 要素 | 現行実装 | 分類 | migration | 備考 |
|---|---|---|---|---|---|
| M1 | サイドナビ5群 | `layout.tsx:56-85`＋side-nav | **実装済(A層)**〔旧: 既存〕 | 否 | 現行は領収書・ご契約も・2026-09-07 変更なし（差分なし・現行が上位集合） |
| M2 | ブランド N／NOX／CLUB NOX | `layout.tsx:35-36` | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
| M3 | 見出し「ホーム」＋STORE OVERVIEW | `dashboard-board.tsx:144-146` | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（lead 据え置き＝裁定158） |
| M4 | 営業日 | `:60,146` bizDateOf・cutoff `page.tsx:56` | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
...
| M6 | **店舗セレクタ** | 現行ホームは先頭店固定 `dashboard/page.tsx:18`（limit 1）・RLS は owner=org 全店・他5画面にセレクタ前例（report／analytics／payroll／receipts／casts） | **実装済(B層)**〔旧: 準備中（owner 閲覧切替）／新要件（F4 認可切替）〕 | 表示は否／F4 本体は要（memberships 部分 unique drop＋auth_store_id 差替＝CLAUDE.md） | 2層で記録・2026-09-09 B1-a e617eaa（owner 閲覧切替＝裁定192・F4 は第 2 層） |
...
| M8 | 本日の出勤 6/8・未着1・欠勤1 | `:73-74,116`（attendance）＋`:124`（confirmed 件数） | **実装済(A層)**〔旧: 既存（内訳表示差）〕 | 否 | 「未着」明示ステータスなし（late＋eta）・2026-09-07 H2 84c3e82（N/予定N名＋内訳＝裁定157） |
| M9 | 現在不足 あと1人 20:00–22:00 | `:125-133`（staffing_needs×shifts バンド交差） | **実装済(A層)**〔旧: 既存〕 | 否 | 帯ラベル未表示・2026-09-07 H2 84c3e82（帯ラベル＝裁定154） |
...
| M12 | 営業前① キャスト不足→追加配置 | M9 同源・導線 /shift | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 H2 84c3e82（不足時「追加配置へ」＝裁定154） |
| M13 | 営業前② 本日の予約 6件／VIP／同伴 | `reservation-panel.tsx:169-174,231-236`（kpiBooked/kpiVip・dohan＝nom_type） | **新要件（ホーム合成・M11 レーン）**〔旧: 既存（再利用可）〕 | 否 | 導線＝/register 予約タブ・2026-09-07 裁定154＝新規クエリを伴う合成のため A層外（教訓60） |
| M14 | 営業前③ 仮シフト未確認 3名 | `shift-board.tsx:61`（proposed）・`:289,314` | **実装済(A層)**〔旧: 既存〕 | 否 | 件数カウント追加・2026-09-07 H2 84c3e82（仮シフト未確認 N名＝裁定154） |
| M15 | 営業中① 不足／至急→代替を探す | M9＋shift-autoassign／day-add-panel | **実装済(A層)**〔旧: 既存（導線要確認）〕 | 否 | 2026-09-07 H2 84c3e82（導線＝M12 と同一・裁定154） |
| M16 | 営業中② 未打刻超過→連絡 | 判定器完備（punch-match.ts・punch-io.ts・punches）・ホーム表示なし | 新要件（表示・器あり） | 不要 | **[連絡]の実体（LINE/SMS）は器なし**＝準備中 or tel リンク |
| M17 | 営業中③ 稼働中5卓／空席3卓／VIP1 延長確認 | `register-board.tsx:435,444`（open checks snap）・seats | **新要件（ホーム合成・M11 レーン）**〔旧: 既存（再掲）〕 | 否 | 2026-09-07 裁定154＝新規クエリを伴う合成のため A層外（教訓60） |
| M18 | 営業後① 未締め／実査未入力→日報へ | `report-board.tsx:165-167,198`・counted_cash | **新要件（ホーム合成・M11 レーン）**〔旧: 既存〕 | 否 | 2026-09-07 裁定154＝新規クエリを伴う合成のため A層外（教訓60） |
| M19 | 営業後② 現金差異 ¥-3,000／理由入力・管理者承認 | diff／note 列あり・**承認フローなし** | **実装済(C層③)**〔旧: 表示＝既存／承認＝新要件（器なし）〕 | **要**（＝D20） | | ・2026-09-10 **実装(C層③ 面a a801683)**: ＝D20（cash_diff_approve・理由必須・承認者＝memberships.id・監査 cash_diff_approve） |
| M20 | 営業後③④ 売掛 期限超過／残高 | `report-board.tsx:183,299,45` | **新要件（ホーム合成・M11 レーン）**〔旧: 既存〕 | 否 | 「売掛利用店舗のみ」＝S24/D35・2026-09-07 裁定154＝新規クエリを伴う合成のため A層外（教訓60） |
| M21 | 営業後⑤ 新人の初期設定未完了（LINE／待遇時給） | 待遇＝casts-board `:655,673-674`・LINE＝器なし | 待遇＝既存／LINE＝新要件 | LINE は要 | 定義を待遇未割当＋PIN 未設定に限れば不要 |
| M22 | 今日のシフト帯（4分割数値） | `:203-235`（atts から算出可） | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 H2 84c3e82（4分割＝裁定154） |
| M23 | 時間帯バー（あと1人／充足） | `:125-133` todayBands（**計算済み・未描画**） | **実装済(A層)**〔旧: 既存〕 | 否 | map で展開・2026-09-07 H2 84c3e82（時間帯バー・Warning 系＝裁定154/159） |
| M24 | 本日の予約リスト | `reservation-panel.tsx:242-245` upcoming（そのまま移植可） | **新要件（ホーム合成・M11 レーン）**〔旧: 既存〕 | 否 | 2026-09-07 裁定154＝新規クエリを伴う合成のため A層外（教訓60） |
| M25 | クイック操作4枚 | `dashboard/page.tsx:36-50`（現行9枚） | **実装済(A層)**〔旧: 既存〕 | 否 | 配列を削る・2026-09-07 H1 8e2eeff（見出し・文言のみ＝裁定155） |
| M26 | 営業状況（売上／前営業日比／組数／本指名／同伴） | M7 同源＋cmp（analytics `:427`）＋nom 件数 | 新要件（合成） | 不要 | ★当日ライブ vs 前日締め済み＝分母の性質差を注記 |
| M27 | 本指名ランキング（今月 1〜3位） | `:78-80,93,242-257` get_cast_ranking | **実装済(A層)**〔旧: 既存〕 | 否 | 上位5→3・2026-09-07 H1 8e2eeff（見出しのみ＝裁定156） |
| M28 | お知らせ最新2件 | `:86-88,260-272` | **実装済(A層)**〔旧: 既存〕 | 否 | 完全一致・2026-09-07 変更なし（差分なし） |
...
| # | v2.1 要素 | 現行実装 | 分類 | migration | 備考 |
|---|---|---|---|---|---|
| A1 | 見出し「売上・店舗分析」 | AB:516-517 | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 N1 ad44954（lead＝裁定160） |
| A2 | CSV出力（ヘッダ） | AB:98-106／478-495（各パネル内） | **実装済(据え置き・裁定166)**〔旧: 既存（位置差）〕 | 否 | ランキング CSV の「売上（按分）」列は A15 に従い改訂・2026-09-07 CSV 位置は A15 に従属 |
| A3 | 今月／先月 | AB:521-530 | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
...
| A5 | 月 input | AB:531 | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
| A6 | 比較 前月／前年同月／なし | 前月同期のみ AB:401-403,427・前年同月は month-report `:83` 前例 | 準備中 | 否 | |
| A7 | 「締め済み営業日のみ」注記4行 | 常時締め済み固定（daily_reports 直読 AB:192-196・`:552-554`） | **実装済(A層)**〔旧: 既存〕 | 否 | 速報導線は Link のみ・2026-09-07 N1 ad44954（注記3行＋速報導線＝裁定161） |
| A8 | 4ビュータブ | AB:607-611 | **実装済(A層)**〔旧: 既存〕 | 否 | 完全一致・2026-09-07 変更なし（完全一致） |
| A9 | （mock に無い）店舗セレクタ／全店合算 | AB:532-551 | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（残置） |
| A10 | **会計売上**＝確定売上 | daily_reports（0010）＋salesOf（AB:90-91）／明細側 store_category_aggregate（0096:137-192） | **実装済(A層)**〔旧: 既存〕 | 否 | 日報（丸め後）と明細Σ（丸め前）非一致は既注記 AB:696-700・2026-09-07 変更なし（式不触） |
...
| A12 | **指名実績**＝本／場内／同伴 | get_cast_ranking（0016:246-324）は checks.nom_type（伝票代表種別）・cast_sales_aggregate（0124）は名簿行 nom_kind＝**2系統併存**・店合計 get_store_nom_counts（0054）は nom_type 側 | **実装済(A層)**〔旧: 既存（2系統）〕 | 望ましい（nom_kind へ統一） | 画面は注記で逃げている AB:1032-1034・2026-09-07 変更なし（2系統統一は要点26＝A層外） |
...
| A16 | 売上／前月比 | AB:561-567 | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし |
| A17 | 組数 | AB:568-572（slips） | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
| A18 | 客単価 | AB:573-577「組単価」 | **実装済(据え置き・裁定163)**〔旧: 既存（語彙差）〕 | 否 | 客数基準なら guests・2026-09-07 組単価のまま |
...
| A22 | 売上推移（今月／前月重ね） | AB:771-843 | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（現行が上位集合） |
...
| A27 | 売上カテゴリ5分類 | store_category_aggregate→category-map.ts→AB:676-703 | **実装済(据え置き・裁定162)**〔旧: 既存〕 | 否 | 表示順のみ・2026-09-07 別掲・固定順のまま |
...
| A31 | 時間帯別（2h バケット・25時以降） | store_hourly_aggregate（1h 刻み）→hourBars AB:442-451 | **実装済(据え置き・裁定164)**〔旧: 既存（再形）〕 | 否 | 2026-09-07 1h 刻みのまま |
| A32 | 支払方法（現金／カード／その他） | payMix AB:411-417（現行4分類＝売掛独立） | **実装済(据え置き・裁定165)**〔旧: 既存〕 | 否 | 2026-09-07 4分類のまま |
| A33 | 日別実績テーブル（人件費率列） | 日別バーあり・表なし・**日別人件費率は器なし**（payslips 月次） | 新要件（日別人件費率） | 否 | forecastDay なら日別概算可 |
| A34 | 「月報へ」 | 無し | 新要件（Link） | 否 | |
| A35 | （mock に無い）曜日×時間帯ヒートマップ | AB:926-963 | **実装済(A層)**〔旧: 既存（残置判断）〕 | 否 | 2026-09-07 変更なし（残置） |
...
| A44 | （mock に無い）主要客リスト | get_cast_customer_ranking（0031）＋AB:1037-1113（件数のみ・金額なし） | **既存（PII ゲート待ち・不触）**〔旧: 既存（残置推奨＝按分禁止と整合）〕 | 否 | 2026-09-07 顧客名表示は弁護士ゲート後 |
...
| A53 | 按分禁止と check_cast_backs の関係 | 商品バックは close 時に**在席 cast へ数量按分**（0132/0133 最大剰余）＝キャスト按分であり顧客売上按分ではない | **実装済(A層)**〔旧: 既存（衝突しない）〕 | 否 | 「販売実績＝同腕按分の凍結値」と定義文で明記・2026-09-07 変更なし（設計整合） |
| A54 | 客層の内訳（新規／リピート／離反リスク） | segs AB:502-511＋churn_tier（0030:79-85） | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（累計 ¥ は A52 論点＝不触） |
...
| A59 | 「顧客管理へ」 | AB:1141 | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
| A60 | （mock に無い）初来店月ごとの再来店表 | AB:1148-1188＋cohort | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（残置） |
...
| # | v2.1 要素 | 現行実装 | 分類 | migration | 備考 |
|---|---|---|---|---|---|
| N1 | 見出し＋リード | `notices-board.tsx:214-216` | **実装済(据え置き・裁定169)**〔旧: 既存〕 | 否 | 2026-09-07 lead は LINE/メール語のため据え置き |
...
| N9 | 「在籍中を対象・休店/退店除外」注記 | `page.tsx:24-27` count＋`:336-339` 注記 | **実装済(据え置き・裁定171)**〔旧: 既存（注記のみ）〕 | 否 | 2026-09-07 休店＝第2期語 |
| N10 | カテゴリ5種 | `:262-269` opacity 0.35 表示のみ（列なし） | 準備中 | 要（category 列） | |
| N11 | 件名 | `:288-294`（80・0034:90） | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
| N12 | 本文 | `:295-302`（4000・0034:92） | **実装済(A層)**〔旧: 既存〕 | 否 | モック 1000 とは差（実装が正）・2026-09-07 変更なし（4000 が正） |
| N13 | 対象 全員／キャスト／黒服（人数） | `:306-340` | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
...
| N21 | 配信方式 アプリ内のみ | 現行の唯一の挙動（掲示のみ） | **実装済(A層)**〔旧: 既存（既定固定）〕 | 否 | 2026-09-07 変更なし（既定固定） |
| N22 | チャネル アプリ内（マイページ） | `app/mine/notices/page.tsx:22-26` | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
| N23 | チャネル LINE | 器ゼロ | 新要件（器なし） | 要 | 外部 API レーン |
| N24 | チャネル メール | 器ゼロ（mail_disabled） | 新要件（器なし） | 要＋インフラ（Resend 等の採否） | |
| N25 | 掲載期間 | `:23,346-351`（until） | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
...
| N29 | 配信対象を確認 | `:381-386→532-557` | **実装済(据え置き・裁定170)**〔旧: 既存〕 | 否 | 2026-09-07 掲載/配信は読み替え済み |
| N30 | 今すぐ配信 | `:554→post()→notice_create` | **実装済(据え置き・裁定170)**〔旧: 既存〕 | 否 | 2026-09-07 今すぐ掲載のまま |
| N31 | LINE プレビュー | `:394-417`（表示のみ） | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（表示のみ） |
| N32 | テンプレート5件 | `:34-50` 3件 | **実装済(A層)**〔旧: 既存（部分）〕 | 否 | 2件追加＝定数・2026-09-07 O2 2fb16d3（5種＝裁定168） |
| N33 | 履歴検索 | `:452-456` | **実装済(A層)**〔旧: 既存〕 | 否 | 2026-09-07 変更なし（差分なし） |
...
| N44 | 「送信失敗してもアプリ内通知は残る」 | 現行は掲示のみ＝結果的に成立 | **実装済(A層)**〔旧: 既存（副次）〕 | 否 | 2026-09-07 変更なし（副次） |
...
## §13 給与管理 v2.1（W）— ⑬追補 v1.3（`nox-payroll-redesign-v2_1.html`・`7029b494…3a20`・29,910B・収蔵済み）

前提: 確定粒度＝**run 単位（店×期間・暦月固定）**＝`payroll_finalize` は payslips を全件 delete→insert（0016:431-443）・`payroll_runs (store_id, period)` unique・status draft/finalized/paid の3値・blocker 1件で route が 422（finalize/route.ts:22-28）・既裁定「確定単位は run のまま／キャスト単位確定は作らない」（U1 設計書:17・ui-calc.ts:36-38）。`payroll_reopen`（0060・D-1）＝run 全体 finalized→draft・payment_records が1件でもあれば拒否・paid はロック・天引き巻き戻し＋audit。**手動調整の器なし**（裁定99-⑥「採用しない・準備中表示もしない」・`wage_adjustments` は repo 未収蔵＝Downloads の 0138 は未適用）。集計元＝**checks.status='closed'・shifts confirmed・punches**（daily_reports 非参照・claim と指名料率は close 非依存）。

| # | v2.1 要素 | 現行実装 | 分類 | migration | 備考 |
|---|---|---|---|---|---|
| W1 | hero「給与管理」 | `payroll-board.tsx:362-369` | **実装済(A層)**〔旧: 既存〕 | 不要 | 裁定99-①・2026-09-07 O3 ac73128（勤怠＝裁定172） |
| W2 | ヘッダ「要確認 N件」 | `:500-520`＋ui-calc.ts:63 issuesOfDraft | **実装済(据え置き・裁定177)**〔旧: 既存（位置差）〕 | 不要 | 2026-09-07 位置は据え置き |
| W3 | 店舗セレクタ | `:376-380` | **実装済(A層)**〔旧: 既存〕 | 不要 | 2026-09-07 変更なし（差分なし） |
| W4 | 給与期間セレクタ（**半月**） | `:385` type=month・window.ts:32-58 period_bounds・runs.period は `^\d{4}-\d{2}$`（0016:124） | **新要件** | **要**（period_bounds・run unique・payslips.period 全面＝money-core） | |
| W5 | 給与締めサイクル（15日・月末） | なし（comp-methods.ts:19「保証判定単位（半月・日）」準備中） | 新要件 | 要 | |
| W6 | 支払予定日 | なし（runs に pay_date なし） | 新要件 | 要 | |
| W7 | 再集計 | `:387`→/api/payroll/preview→core.ts:117 | **実装済(据え置き・裁定173)**〔旧: 既存（名称差）〕 | 不要 | 毎回フル再計算・2026-09-07 プレビューのまま |
| W8 | **給与グループ**（キャスト／正社員／日払い・サイクル別・支払日別） | なし（pay_group はレジの会計分割＝無関係） | **新要件** | **要**（グループ表＋cast 割当＋グループ別 run＝W4-W6 と一体） | |
| W9 | 4ステップ | `:399-449`（STEPS） | **実装済(A層)**〔旧: 既存〕 | 不要 | 4段目「支払・明細」（LINE 公開は裁定99-⑦で非表示）・2026-09-07 変更なし（4段目は裁定99-⑦） |
| W10 | KPI 対象者／総支給／控除／今回支払 | `:455-491`・ui-calc.ts:21・sum4 | **実装済(据え置き・裁定174)**〔旧: 既存〕 | 不要 | CSV と逐語同一・2026-09-07 W11 定義変更前 |
| W11 | KPI **既払額**（日払い・前渡し）を控除と分離 | 前借り＝advances を**控除**として net から減算（collect.ts:381-407・core.ts:196） | **新要件（定義変更）** | 要（見直し） | ★会計定義変更＝要裁定（S-1 系） |
| W12 | 5タブ | 縦1ページ＋下段2見出し（`:772,776`） | **実装済(据え置き・裁定177)**〔旧: 既存（再配置）〕 | 不要 | 履歴タブは W41・2026-09-07 5タブ化は再配置＝無人外 |
| W13 | 集計元「勤怠・シフト 7/7 取込済み」 | collect.ts:295-338 loadPunch（confirmed・punches・attendance） | **既存（表示欠落・停止リスト）**〔旧: 既存（可視化なし）〕 | 不要 | 2026-09-07 preview 応答に取込状態なし＝要 API 確認 |
| W14 | 集計元「売上・バック **締め済み日報のみ**」 | collect.ts:219-222（checks closed）・daily_reports **非参照**・claim/料率は close 非依存 | **新要件（定義差）** | 要（依存を足すなら） | ★日報 v2.1 との結線は裁定要 ・2026-09-10 #70: 日報の**再締めは締め欄経由**（解除中の日を選ぶと締め欄が「再締め」になり入力値を daily_report_reclose へ送る・表の行ボタンは締め欄への誘導のみ） |
| W15 | 集計元「控除・前借り 要確認」 | collect.ts:345/381/411（自動天引き・件数は core が保持） | **既存（表示欠落・停止リスト）**〔旧: 既存（表示欠落）〕 | 不要 | 「要確認」状態なし・2026-09-07 要確認状態の器なし（W19） |
| W16 | 集計元「税・契約区分 未設定＝不足」 | core.ts:41 Blocker（no_plan/no_tax/no_employment）・ui-calc.ts:55-57 | **実装済(A層)**〔旧: 既存〕 | 不要 | 2026-09-07 変更なし（差分なし） |
| W17 | 前回集計後の変更検知（打刻訂正） | なし（前回結果を保存しない） | 新要件 | 要（前回スナップショット） | |
| W18 | 確定前の要確認リスト | `:500-520`＋ui-calc.ts:63-79 | **実装済(A層)**〔旧: 既存〕 | 不要 | 2026-09-07 変更なし（差分なし） |
| W19 | 要確認の操作（本人設定へ／内訳／確認） | href は no_plan のみ・「確認」の状態保存先なし | 準備中（一部） | 要（確認記録なら） | 類似＝deductions.basis_confirmed_at（裁定98） |
| W20 | 入店時給保証（適用期間別 5,000→4,500） | pay.ts:572-578 guaranteeAdd＝**期間総額の床**・wageDetail は日次 | 準備中（定義差） | 要 | 日付範囲つき時給レート保証の器なし（＝K33）・**S-1 系** |
| W21 | 売掛負担を控除表示 | receivables・core.ts:197・`:620` | **実装済(A層)**〔旧: 既存〕 | 不要 | 2026-09-07 変更なし（差分なし） |
| W22 | 対象者リスト（検索・チップ絞込） | 検索 `:576-577,597`・チップなし（taxMode/employment は行にあり） | **実装済(A層)**〔旧: 既存（表示差）〕 | 不要 | 2026-09-07 O3 ac73128（チップ＝裁定175） |
| W23 | 行の総支給／控除／差引／稼働 h（日数） | `:597-624`（日数列なし・wdays は保有） | **実装済(A層)**〔旧: 既存（軽微）〕 | 不要 | 2026-09-07 O3 ac73128（日数列＝裁定176） |
| W24 | **スタッフ（黒服／雇用）を給与対象に含む** | 対象＝casts のみ（collect.ts:532-543）・staff は非対象 | **新要件** | **要** | ＝H3/K51 と同じ「cast／staff 二軸」 |
| W25 | 内訳ドロワー | 右 sticky `:658-740`＋PayslipSlip | **実装済(A層)**〔旧: 既存〕 | 不要 | 適用期間別時給行は W20 依存・2026-09-07 変更なし（差分なし） |
...
- **(2) 確定後の訂正と D-1**: 「A 解除→訂正→再確定」は `payroll_reopen` で概ね充足（＝**D-1 と同族＝同レーン**）。ただし run 全体・一部支払済みで全員分が解除不能・理由/承認者列なし。「B 次回精算」は器ゼロ（W26 と同じ）。
- **(3) 手動調整の器**: なし（裁定99-⑥で明示的に不採用）。pay.ts の extras は出勤インセンティブ由来のみ。採るなら `payroll_adjustments`（run×cast・符号付・reason・created_by・approved_by）＋finalize 取込＋reopen 巻き戻し。**実装順の含意＝W26→W28→W29/W30 の順でしか積めない**（調整行がないと訂正再確定に入れる値が無く、per-cast 確定がないと「保留」が定義できない）。
- **(4) 支払状況／振込データ／源泉納付**: 支払＝payment_records で複数回・複数方法対応済み（予定行なし）。振込データ＝**器なし・口座列なし＝専門家ゲート待ち**（機微）。源泉納付＝withholding_* でモック7列と一致・**母数 paid のみ＋mark_paid の UI 不在＝運用穴**・順延なし＝**税理士ゲート待ち**（裁定23／T1-T3）。
- **(5) 集計元の状態＝締め済み依存**: **依存していない**（checks closed・shifts confirmed・punches・daily_reports 非参照・claim/料率は close 非依存）。モックの「締め済み日報のみ／未締め日があると計算不可」は**現行と定義が違う新要件**＝日報 v2.1（X2・裁定要点23/24）と結線するかを裁定。

**§13 総括**: 表示層で載るもの＝W2/W12/W13/W15/W22/W23/W41（履歴は audit 読み）。money-core 別レーン＝W4-W6/W8（半月・グループ＝period_bounds と run unique）／W26・W28・W29・W31（手動調整→per-cast 確定→中間状態→次回精算）／W11（既払額の会計分離）／W20（期間付き時給保証＝K33 と同件）／W24（スタッフ給与＝二軸）。専門家ゲート待ち＝W38（口座）・W20/W11（S-1）・W39（順延・税理士）。
...
- v1.2（最終ラウンド4面）は `nox-design-pack-v1_2.zip`（`5748901c…020c`・台帳 v1_2 `42656de8…7e55`）として照合収蔵。v1.3（給与 v2.1）は `nox-design-pack-v1_3.zip`（`880b2a7b…905c`・台帳 v1_3 `3d976afd…c4e0`）として照合収蔵＝**11ページでメニュー全面のデザイン受領完了**（台帳 v1_3）。A 系の行番号は `analytics-board.tsx` を AB と略記。
- 裁定要点の番号は追記順（31〜36 は 30 の前に置いた＝給与を通知の直前に配置）。
