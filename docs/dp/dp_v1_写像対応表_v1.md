...
| # | v2.1 要素 | 現行実装 | 分類 | migration | 備考 |
|---|---|---|---|---|---|
| W1 | hero「給与管理」 | `payroll-board.tsx:362-369` | 既存 | 不要 | 裁定99-① |
| W2 | ヘッダ「要確認 N件」 | `:500-520`＋ui-calc.ts:63 issuesOfDraft | 既存（位置差） | 不要 | |
| W3 | 店舗セレクタ | `:376-380` | 既存 | 不要 | |
...
| W7 | 再集計 | `:387`→/api/payroll/preview→core.ts:117 | 既存（名称差） | 不要 | 毎回フル再計算 |
| W8 | **給与グループ**（キャスト／正社員／日払い・サイクル別・支払日別） | なし（pay_group はレジの会計分割＝無関係） | **新要件** | **要**（グループ表＋cast 割当＋グループ別 run＝W4-W6 と一体） | |
| W9 | 4ステップ | `:399-449`（STEPS） | 既存 | 不要 | 4段目「支払・明細」（LINE 公開は裁定99-⑦で非表示） |
| W10 | KPI 対象者／総支給／控除／今回支払 | `:455-491`・ui-calc.ts:21・sum4 | 既存 | 不要 | CSV と逐語同一 |
| W11 | KPI **既払額**（日払い・前渡し）を控除と分離 | 前借り＝advances を**控除**として net から減算（collect.ts:381-407・core.ts:196） | **新要件（定義変更）** | 要（見直し） | ★会計定義変更＝要裁定（S-1 系） |
| W12 | 5タブ | 縦1ページ＋下段2見出し（`:772,776`） | 既存（再配置） | 不要 | 履歴タブは W41 |
| W13 | 集計元「勤怠・シフト 7/7 取込済み」 | collect.ts:295-338 loadPunch（confirmed・punches・attendance） | 既存（可視化なし） | 不要 | |
...
| W15 | 集計元「控除・前借り 要確認」 | collect.ts:345/381/411（自動天引き・件数は core が保持） | 既存（表示欠落） | 不要 | 「要確認」状態なし |
| W16 | 集計元「税・契約区分 未設定＝不足」 | core.ts:41 Blocker（no_plan/no_tax/no_employment）・ui-calc.ts:55-57 | 既存 | 不要 | |
| W17 | 前回集計後の変更検知（打刻訂正） | なし（前回結果を保存しない） | 新要件 | 要（前回スナップショット） | |
| W18 | 確定前の要確認リスト | `:500-520`＋ui-calc.ts:63-79 | 既存 | 不要 | |
...
| W21 | 売掛負担を控除表示 | receivables・core.ts:197・`:620` | 既存 | 不要 | |
| W22 | 対象者リスト（検索・チップ絞込） | 検索 `:576-577,597`・チップなし（taxMode/employment は行にあり） | 既存（表示差） | 不要 | |
| W23 | 行の総支給／控除／差引／稼働 h（日数） | `:597-624`（日数列なし・wdays は保有） | 既存（軽微） | 不要 | |
| W24 | **スタッフ（黒服／雇用）を給与対象に含む** | 対象＝casts のみ（collect.ts:532-543）・staff は非対象 | **新要件** | **要** | ＝H3/K51 と同じ「cast／staff 二軸」 |
| W25 | 内訳ドロワー | 右 sticky `:658-740`＋PayslipSlip | 既存 | 不要 | 適用期間別時給行は W20 依存 |
...
| W27 | 確定時スナップショット固定 | payslips.breakdown_json 凍結（0016:149）・cast 名凍結 | 既存 | 不要 | |
...
| W30 | 確定後の訂正 A（解除→訂正→再確定） | payroll_reopen（0060:37-156・finalized のみ・paid 不可・payments exist 拒否）＋owner route＋UI `:746-767` | 既存（＝**D-1 同族・概ね充足**） | 不要 | ★run 全体・支払1件でも不可＝「対象者だけ訂正」は W28 依存 |
...
| W33 | 支払状況（複数回・複数方法・残額・状態） | payment_records（0021:306-323）・payment_record_add（Σ≤net・finalized/paid 限定）・payment-panel.tsx:103-163・payStatusOf | 既存 | 不要 | 「予定行」は器なし（実績のみ） |
| W34 | 明細公開（LINE／メール／アプリ内・自動通知） | なし＝裁定99-⑦・T3 後送り（`:397,770`）・cast 側は /mine の payslips 直近6期（RLS） | 準備中（既裁定） | 要（公開フラグ＋T3） | ＝N40 自動通知 |
| W35 | 給与明細 CSV | `:183-230,776-797`＋csv.ts:35-71 | 既存 | 不要 | |
| W36 | 報酬明細／PDF | `:234-256,802-840`＋payslip-slip | 既存 | 不要 | |
| W37 | 支払明細・請求関連（委託の分離） | invoice-panel.tsx＋tax-overview／tax-report-csv（owner） | 既存 | 不要 | |
...
| W39 | 源泉徴収・納付管理（7列） | withholding_monthly_summary（0075:79-127・期限＝翌月10日）・withholding_payment_record（owner）・payment-tax-panel.tsx:97-148 | 既存（ほぼ逐語一致） | 不要 | ★母数＝**paid run のみ**（0075:119）だが `payroll_mark_paid` を呼ぶ UI/route が**不在**（verify のみ）＝運用の穴／未凍結行は記録不可／納期特例・順延なし＝裁定23（**税理士ゲート待ち**・台帳:832） |
| W40 | 源泉額の計算根拠 | pay.ts:441 withholdingOf＝floor(max(0, gross−5000×periodDays)×0.1021)・periodDays（裁定23-b） | 既存 | 不要 | |
