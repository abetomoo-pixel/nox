...
| D25 | ③再締め（再確定日時・担当者） | reclose は reclosed_count++ のみ（0055:497）・reclosed_at/by 列なし | **実装済(C層③)**〔旧: 準備中〕 | **要**（列で持つなら） | UI `:827`・B2 対象外（2026-09-09）＝器なし・C層③／設計書行き ・2026-09-10 **実装(C層③ 面a a801683)**: reclosed_at／reclosed_by／reclose_idem_key（mig0138）＝再締めは解除中のみ・冪等。/report は解除中バッジ＋再締め者名（memberships→users） ・2026-09-10 #70: 日報の**再締めは締め欄経由**（解除中の日を選ぶと締め欄が「再締め」になり入力値を daily_report_reclose へ送る・表の行ボタンは締め欄への誘導のみ。W14 備考から移動） |
...
| W14 | 集計元「売上・バック **締め済み日報のみ**」 | collect.ts:219-222（checks closed）・daily_reports **非参照**・claim/料率は close 非依存 | **新要件（定義差）** | 要（依存を足すなら） | ★日報 v2.1 との結線は裁定要 |
