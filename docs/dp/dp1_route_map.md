...
★「あり」は区画の存在判定。移植完了ではない（教訓42・2026-08-27）

**#7.5 デザイン移植レーン（DP）** の画面対応の正本。母数は **33ルート**（DP0-5）。
変換規模は **構造差基準**（意匠差ではない）＝**DP1 = 構造変換** ／ **DP2 = 意匠仕上げ**。

調査の裏付け: `docs/dp/dp0_survey.md`（DP0 調査）＋`docs/dp/dp1_structure_survey.md`（DP1 構造サーベイ）。
...
| 台帳 | `mock_pages-2026-08_sha256.txt` への追記は **未実施**（前回 DP1 指示の破棄に伴い revert 済み）。追記文面案は `docs/dp/dp0_survey.md` §S6-3 |
