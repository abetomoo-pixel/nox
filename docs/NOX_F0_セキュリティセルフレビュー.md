...
| 34 | drink_claims の back_amount が can_register staff に可視（**既知穴・mig0038 レビュー起票 2026-07-15**） | mig0038 レビュー指摘2（相談役裁定） | mig0037 の `drink_claims_select` staff 枝は `can_register` のまま＝承認済み claim の `back_amount`（キャスト別ドリンクバック額）が can_register staff に可視。mig0038（check_cast_backs の staff 可視を専用フラグ `can_view_backs` に分離）の**対象外と裁定**・mig0038 では drink_claims に触らない。**F3f UI 設計時に staff 枝を再裁定**（候補＝`status='pending'` 限定 or 焼付け済み行に `auth_staff_can_view_backs()` 要求）。★**F3f を本番投入する場合はこの裁定完了を本番手貼りの前提条件とする** |
| 35 | trials（体入採用・mig0040）の見送り後 個人情報保持期間（**F3d 起票 2026-07-15・弁護士ゲート**） | F3d 設計ロック⑤（相談役裁定） | `trial_reject`（見送り）後も trials 行の `real_name`/`birthday` は DB に残置（削除 RPC は**作らない**＝mig0040 の意図）。個人情報の保持期間・消去義務（個情法/風営法の年齢確認記録との関係）は**弁護士確認後に裁定**し、必要なら消去 RPC（マスク audit 流儀）を後続 mig で追加。それまで trials の可視は owner/manager 限定 RLS で最小化 |
