-- docs/tmp/live_c3_open.sql — check_open の全文と checks 全列（C3/C4 挙動段の底本）
-- 貼り先証明: {"t":"nox-project-proof","n":3} / 取得日 2026-08-28 / dev DB mig0001〜0112
-- pg_get_functiondef 逐語（LF 正規化のみ）

-- ============ 1. check_open（overload 1 本） ============
--   check_open(uuid,integer,text,uuid,uuid)  secdef=true  acl={postgres=X/postgres,authenticated=X/postgres,service_role=X/postgres}  cfg={search_path=public}
...
-- ============ 2. checks の全列（29 列・ordinal_position 順） ============
--   # 1 id                     uuid                       null=NO default=gen_random_uuid()
--   # 2 org_id                 uuid                       null=NO default=-
--   # 3 store_id               uuid                       null=NO default=-
--   # 4 seat_id                uuid                       null=NO default=-
--   # 5 status                 text                       null=NO default='open'::text
--   # 6 started_at             timestamp with time zone   null=NO default=now()
--   # 7 people                 integer                    null=YES default=-
--   # 8 nom_type               text                       null=NO default='free'::text
--   # 9 customer_id            uuid                       null=YES default=-
--   #10 merged_into            uuid                       null=YES default=-
--   #11 total                  integer                    null=NO default=0
--   #12 service_rate           integer                    null=NO default=-  -- ★開卓時凍結（既存3列）
--   #13 round_unit             integer                    null=NO default=-  -- ★開卓時凍結（既存3列）
--   #14 round_mode             text                       null=NO default=-  -- ★開卓時凍結（既存3列）
--   #15 close_idem_key         uuid                       null=YES default=-
--   #16 closed_at              timestamp with time zone   null=YES default=-
--   #17 voided_at              timestamp with time zone   null=YES default=-
--   #18 voided_by              uuid                       null=YES default=-
--   #19 void_reason            text                       null=YES default=-
--   #20 created_by             uuid                       null=NO default=-
--   #21 created_at             timestamp with time zone   null=NO default=now()
--   #22 updated_at             timestamp with time zone   null=NO default=now()
--   #23 set_min                integer                    null=NO default=60  -- ★凍結スナップ系
--   #24 set_fee                integer                    null=NO default=0  -- ★凍結スナップ系
--   #25 ext_min                integer                    null=NO default=30  -- ★凍結スナップ系
--   #26 ext_fee                integer                    null=NO default=0  -- ★凍結スナップ系
--   #27 time_per               text                       null=NO default='table'::text
--   #28 dohan_fee              integer                    null=YES default=-  -- ★凍結スナップ系
--   #29 ext_menu_snap          jsonb                      null=YES default=-
