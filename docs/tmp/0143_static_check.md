...
## 補足（insert 列数・diff 式の複数行・末尾 ;）
b. ★insert 文が取れない
   live: 列 24 / values 28
d. daily_report_close diff 式: live L50〜52 / mig ブロック内 L54〜56 → 一字一致・v_ar_card/other: 含まない
...
d. daily_report_reclose diff 式: live L39〜40 / mig ブロック内 L42〜43 → 一字一致・v_ar_card/other: 含まない
   v_diff := case when v_counted is null then null
   else v_counted - (v_float + (v_agg->>'cash')::int + v_ar - v_expense - v_payout) end;
末尾 `end $function$;` の差＝pg_get_functiondef は文終端の ; を出さない（mig 側は必要）＝本文差ではない
