...
## I1. errBox（t.alert）の面3値 →★E5c で裁定済＝トークン新設（案aを採用）
...
- **★E5c 裁定反映（2026-08-17）**: 判定基準「実使用3箇所以上でトークン新設」→ 実測
  **10箇所**（9ファイル・invoice-panel:116 はコメント言及のみ＝除外）で新設確定。
  指示の明示は `--bad-bg/--bad-bd` の2つだが、文字色 #F0B9B9 だけリテラルに残すと
  是正が半端になるため**案aどおり `--bad-ink` を含む3トークン**で実装（値は従来と同一＝
  見た目不変・G3 監査で同名衝突ゼロ・ガイド §1-2 収載・print 上書き不要の理由も収載）。
  付随: casts-board:558 / staff-board:271 の danger ボタン枠 `#5A2E2E` 2箇所も
  `--bad-bd` へ参照化。products-board:892/900 の `rgba(220,80,80,.10)` 上書きは
  ローカル意匠（薄面変奏）として不触＝吸収は見送り。
...
## E5c で置換・是正したもの（ギャップではない・記録のみ）

- **I1 裁定反映**は I1 節に追記のとおり（3トークン新設・参照12箇所化）。
- **report 圏（report-board / month-report / page）**: 旧パレットリテラル**ゼロ**・
  締め済み日報の全列表は E3 の t.th/t.td 使用済み。置換は**売掛行（report-board:202）
  → `.nox-listrow`（G5 部品）の1箇所のみ**＝G5 起草時の「同型8箇所/7ファイル」走査から
  漏れていた同型（B6 で後から追加された行のため）。wrap/gap8/padding9 はローカル上書き・
  罫は旧 borderTop（見出し直下にも線）から部品の border-bottom（最終行なし）へ＝部品の
  線位置に統一（E4 の shift/wishes 適用と同じ扱い）。
- **不触と判断したもの（再発明ではない）**:
  - month-report:108-109 のローカル th/td 定数＝4列グリッド（display:contents）用の
    右寄せ・罫なし・padding 違い＝t.th/t.td（表・罫あり・左寄せ）とは**意匠が別**。
  - report-board:286 のプレビュー横並び密度ストリップ（padding 4px 12px・縦罫）＝
    H1 と同じ「密度が意匠」型。対応部品なし・トークンのみ使用のため gaps 起票はしない。
  - month-report:147 の売上半期バー（card2 地＋gold 面）＝在庫特化の .nox-stockbar とは
    意味も配色も別。トークンのみ使用。

## E5c 検収の制約（データ都合・構造確認で代替）

- 売掛タブ: open receivables 0件＝`.nox-listrow` の実描画は未確認（検収データを作ると
  receivables/日報に痕が残るため見送り）。部品クラス自体は E4 の shift/wishes 実機確認済み・
  logic check OK・tsc/build 緑で代替。
- t.alert（--bad-* 参照化後）の実描画: 到達には書込操作（PW 再発行・CSV 失敗・締めブロッカー等）
  が要るため見送り。値はトークン＝旧リテラルの同値定義（実機で --bad-bg/#2C1B1B ほか3値の
  解決を確認済み）＝画面不変は構成上保証。
- 日報実機は 2026-07-24（プレビュー 4組 ¥518,400・カードTAX ¥23,310）と月報 2026-07
  （指名18本・人件費 ¥276,145=paid run 一致）で表示・過去日/月切替のみ確認。締め/再締め/回収/
  天引きの操作なし。
