// 送り実費発行（manager+）。transport_issue を呼ぶ（RPC が二重防御＝manager+ 検証・org 照合・
//   okuri_mode='actual' 必須[flat 店は拒否＝L3' 構造的排他]・paid 期間ガード・cast 照合・audit）。
...
export async function POST(req: Request) {
  ...
  let body: { storeId?: unknown; castId?: unknown; amount?: unknown; bizDate?: unknown; note?: unknown };
  try {
    body = await req.json();
  } catch {
    return NextResponse.json({ error: "bad json" }, { status: 400 });
  }
  const { storeId, castId, amount, bizDate, note } = body;
  ...
  const { data, error } = await supabase.rpc("transport_issue", {
    p_store_id: storeId, p_cast_id: castId, p_amount: amount, p_biz_date: bizDate, p_note: noteVal,
  });
  if (error) {
    const m = error.message;
    const status = m.includes("forbidden") ? 403 : m.includes("paid period") || m.includes("okuri not actual") ? 409 : 400;
