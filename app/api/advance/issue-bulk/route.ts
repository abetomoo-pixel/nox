// ★裁定302／304（2026-09-25・mig0157）: 前借りの一括発行（manager+）。ユーザー文脈クライアントで adv_issue_bulk を呼ぶ
//   （RPC が二重防御＝店 org 照合・role・件ごと amount／date／paid period／cast・'duplicate cast'・件ごと idem・1 tx・audit）。
//   route は薄い認証＋入力整形＋エラーマッピングのみ（/api/advance/issue と同型）。
...
export async function POST(req: Request) {
  ...
  const { data, error } = await supabase.rpc("adv_issue_bulk", { p_store_id: storeId, p_items: pItems, p_idem_key: idemKey });
  if (error) {
    const m = error.message;
    const status = m.includes("forbidden") ? 403 : (m.includes("paid period") || m.includes("duplicate cast")) ? 409 : 400;
