// 出勤インセンティブ発行（manager+）。ユーザー文脈クライアントで incentive_publish を呼ぶ
//   （RPC が二重防御＝manager+ 検証・org 照合・paid 期間ガード・部分ユニーク排他・audit actor=auth.uid()）。
// route は薄い認証＋入力整形＋RPC エラーの HTTP マッピングのみ。
...
export async function POST(req: Request) {
  ...
  if (error) {
    ...
    const status = m.includes("forbidden") ? 403 : m.includes("paid period") || m.includes("already published") ? 409 : 400;
